...
test("channel management exposes add, refresh, and secret-free export actions", async () => {
    const [pageSource, apiSource] = await Promise.all([
        Bun.file(new URL("../src/pages/admin/channels/channels-page.tsx", import.meta.url)).text(),
        Bun.file(new URL("../src/services/api/auth.ts", import.meta.url)).text(),
    ]);
    const page = compactSource(pageSource);

    expect(page).toContain("AdminExportButton");
    expect(page).toContain("exportAdminChannels");
    expect(page).toContain('label="导出渠道"');
    expect(page).toContain("不含密钥和请求头值");
    expect(page).toContain('loading={loading}');
    expect(page).toContain("刷新");
    expect(page).toContain("添加渠道");
    expect(apiSource).toContain('http.get<AdminChannelExport>("/admin/channels/export")');
    expect(apiSource).toContain('type: "application/json;charset=utf-8"');
});

test("channel management imports JSON snapshots as disabled credentialless channels", async () => {
    const [pageSource, apiSource] = await Promise.all([
        Bun.file(new URL("../src/pages/admin/channels/channels-page.tsx", import.meta.url)).text(),
        Bun.file(new URL("../src/services/api/auth.ts", import.meta.url)).text(),
    ]);
    ...
    expect(page).toContain("importAdminChannels");
    expect(page).toContain('accept=".json,application/json"');
    expect(page).toContain("JSON.parse(await file.text())");
    expect(page).toContain('message.warning("导入的渠道默认停用且不含密钥，请编辑渠道补充凭证后再启用")');
    expect(page).toContain("await syncChannels(); await reload();");
    expect(page).toContain("导入渠道");
    expect(apiSource).toContain('http.post<{ importedChannels: number; importedModels: number }>("/admin/channels/import", input)');
});
...
test("user account menus do not expose the changelog", async () => {
    const [topBarMenu, sidebarMenu] = await Promise.all([
    ...
    expect(topBarMenu).not.toContain("AppChangelogButton");
    expect(sidebarMenu).not.toContain("AppChangelogButton");
});

test("local tools live behind the administrator navigation", async () => {
    const [navigationSource, routeSource, pageSource, settingsSource] = await Promise.all([
        ...
        Bun.file(new URL("../src/pages/admin/local-tools/local-tools-page.tsx", import.meta.url)).text(),
        Bun.file(new URL("../src/pages/settings/index.tsx", import.meta.url)).text(),
    ]);
    expect(navigationSource).toContain('path: "/admin/local-tools"');
    expect(routeSource).toContain('{ path: "local-tools", element: <LocalToolsPage /> }');
    expect(pageSource).toContain("<LocalCliSettings />");
    expect(settingsSource).not.toContain('key: "local-cli"');
});
...
test("task-first settings reveal dependent configuration only after the primary choice", async () => {
    const [storageSource, emailSource, accessSource, phoneSource, featureSource, drawingSource, arkSource, interceptionSource, thirdPartySource, cssSource] = await Promise.all([
    ...
    expect(emailSource).toContain('title="2. 配置 SMTP 连接与发件身份"');
        ...
        expect(phoneSource).toContain('title="2. 是否启用手机验证码验证"');
        ...
        expect(accessSource).toContain('title="3. 是否开放 Linux.do 登录"');

...
test("task-center forwards explicit task/session provider config without inferring a channel from Zustand", async () => {
    ...
    userStore.useUserStore.setState({ user: { id: "admin", username: "admin", displayName: "Admin", role: "admin", status: "active" }, features: { ...userStore.defaultFeatureAvailability, desktopLocalChannelsEnabled: true } });
    ...
    try {
        ...
        userStore.useUserStore.setState({ user: null, features: userStore.defaultFeatureAvailability });

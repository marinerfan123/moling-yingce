...
afterEach(() => {
    ...
    useUserStore.setState({ user: null, features: defaultFeatureAvailability });
    if (originalWindow === undefined) delete (globalThis as { window?: Window }).window;
    else Object.defineProperty(globalThis, "window", { configurable: true, value: originalWindow });
});

function setLocalChannelRuntime(desktopLocalChannelsEnabled: boolean, hostname: string) {
    useUserStore.setState({ user: { id: "admin", username: "admin", displayName: "Admin", role: "admin", status: "active" }, features: { ...defaultFeatureAvailability, desktopLocalChannelsEnabled } });
...
describe("允许本机渠道前端请求合同", () => {
    ...
    test("UI 只在 Backend capability=true 且页面文本 host 精确 loopback 时显示", () => {
        ...
        expect(desktopLocalChannelUiVisible(true, "localhost", "user")).toBe(false);
    });

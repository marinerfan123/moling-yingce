...
export function useLocalDreaminaModelBootstrap(enabled = true) {
    ...
    const clear = useLocalDreaminaModelStore((state) => state.clear);
    useEffect(() => {
        if (!enabled) {
            clear();
            return;
    ...
    }, [clear, connection, enabled, modules, sync]);
}

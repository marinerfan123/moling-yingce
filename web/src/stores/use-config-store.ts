...
import { canUseCustomChannels, useUserStore } from "@/stores/use-user-store";
...
export function useEffectiveConfig() {
    ...
    const customChannelsEnabled = useUserStore((state) => canUseCustomChannels(state.user, state.features.customChannelsEnabled));
    const canUseLocalTools = useUserStore((state) => canUseDesktopLocalTools(state.user, state.features.desktopLocalChannelsEnabled));
    ...
    return useMemo(() => effectiveConfigWithDreamina(effectiveConfigForLocalTools(effectiveConfigForCustomChannels(config, customChannelsEnabled), canUseLocalTools), catalogState, dreaminaModels, canUseLocalTools), [canUseLocalTools, catalogState, config, customChannelsEnabled, dreaminaModels]);
}
...
export function effectiveConfigForLocalTools(config: AiConfig, canUseLocalTools: boolean): AiConfig {
    if (canUseLocalTools) return config;
    const channels = config.channels.filter((channel) => channel.transport !== "local-runtime" && channel.id !== "local:dreamina-cli" && channel.allowLocalChannel !== true);
    const comfyBridgeHasData = Boolean(config.comfyBridge.enabled || config.comfyBridge.bridgeId.trim() || config.comfyBridge.workflowId.trim() || config.comfyBridge.workflows.length);
    if (channels.length === config.channels.length && !comfyBridgeHasData) return config;
    return normalizeConfigSnapshot({ config: { ...config, channels, comfyBridge: { ...defaultConfig.comfyBridge } } }).config;
}

export function effectiveConfigWithDreamina(config: AiConfig, catalogState: "idle" | "loading" | "ready" | "error", dreaminaModels: DreaminaLocalModel[], canUseLocalTools = true): AiConfig {
    if (!canUseLocalTools) return effectiveConfigForLocalTools(config, false);

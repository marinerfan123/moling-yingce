...
import { Pencil, Plus, Power, RefreshCw, Search, Trash2, Upload } from "lucide-react";
...
import { createAdminChannel, deleteAdminChannel, exportAdminChannels, importAdminChannels, listAdminChannels, type AdminChannelExport, updateAdminChannel } from "@/services/api/auth";
...
export default function ChannelsPage() {
    ...
    const importFile = async (event: ChangeEvent<HTMLInputElement>) => {
        ...
        if (!file.name.toLowerCase().endsWith(".json")) {
            message.error("请选择 JSON 渠道配置文件");
            return;
        }
        setImporting(true);
        try {
            const parsed = JSON.parse(await file.text()) as Partial<AdminChannelExport>;
            if (!parsed || !Array.isArray(parsed.channels)) {
                throw new Error("文件不是有效的渠道配置快照");
            }
            const result = await importAdminChannels(parsed as AdminChannelExport);
            await syncChannels();
            await reload();
            message.success(`已导入 ${result.importedChannels} 个渠道、${result.importedModels} 个模型`);
            message.warning("导入的渠道默认停用且不含密钥，请编辑渠道补充凭证后再启用");
        } catch (error) {
            message.error(error instanceof Error ? error.message : "导入渠道配置失败");
        } finally {
            setImporting(false);
        }
    };
    ...
    return (
        <AdminPageFrame
            title="系统渠道"
            description="管理用户可用的系统模型渠道、模型能力与计费配置；导出文件不含密钥和请求头值。"
            actions={
                <div className="flex gap-2">
                    ...
                    <AdminExportButton
                        exportFile={exportAdminChannels}
                        fileName={() => `系统渠道配置-${new Date().toISOString().slice(0, 10)}.json`}
                        label="导出渠道"
                        title="导出全部系统渠道配置（不含密钥和请求头值）"
                        successMessage="渠道配置已导出（不含密钥和请求头值）"
                        errorMessage="导出渠道配置失败"
                    />
                    <Button icon={<Upload className="size-4" />} loading={importing} onClick={() => importFileInput.current?.click()}>
                        导入渠道
                    </Button>
                    <input ref={importFileInput} className="hidden" type="file" accept=".json,application/json" onChange={(event) => void importFile(event)} />
                    <ChannelOrderDialog
                        onSaved={async () => {
                            await syncChannels();
                            await reload();
        ...
        >
            ...
            <Modal
                ...
                title={editingChannel ? "编辑系统渠道" : "添加渠道"}
                open={drawerOpen}
                width={760}

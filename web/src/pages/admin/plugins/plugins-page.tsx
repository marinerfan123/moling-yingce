...
import { isDesktopLocalPluginId } from "@/lib/desktop-local-tools";
...
function mergePlugins(remote: BackendPlugin[]): AdminPluginItem[] {
    ...
    for (const plugin of listRegisteredPlugins()) {
        ...
        const desktopLocal = isDesktopLocalPluginId(plugin.manifest.id);
        byId.set(plugin.manifest.id, {
            ...
            management: {
                ...
                activationScope: application && !desktopLocal ? "user" : "system",
                configurationScope: desktopLocal ? "system" : application ? (plugin.manifest.id === RUNNINGHUB_PLUGIN_ID ? "user" : "none") : "system",
            },
        });
    }

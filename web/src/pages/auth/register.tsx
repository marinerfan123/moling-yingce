...
import { ArrowRight, Info, LockKeyhole, Mail, MessageCircle, Music2, Phone, ShieldCheck, TriangleAlert, UserRound } from "lucide-react";
import { useNavigate, useSearchParams } from "react-router";

import { douyinLoginURL, getAuthSession, getAuthSettings, linuxDOLoginURL, register, sendRegistrationEmailCode, sendRegistrationPhoneCode, weChatLoginURL } from "@/services/api/auth";
...
export default function RegisterPage() {
    ...
    const sendPhoneCode = async () => {
        if (sendingPhone.current || phoneCountdown > 0) return;
        if (!phone.trim()) {
        ...
        try {
            await sendRegistrationPhoneCode(phone.trim());
            setPhoneCountdown(60);
            message.success("验证码已发送，请查收短信");
        } catch (error) {
            if (error instanceof ApiError && error.status === 429) setPhoneCountdown(Math.max(1, Math.ceil((error.retryAfterMs ?? 60000) / 1000)));
            message.error(error instanceof Error ? error.message : "发送验证码失败");
        } finally {
    ...
    const submit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        setSubmitting(true);
        try {
            await register({ username, email, emailCode, phone, phoneCode, displayName, password });
    ...
    const phoneUnavailable = Boolean(settings && !settings.firstUser && settings.phoneCodeRequired && !settings.phoneVerificationAvailable);
    const disabled = registrationClosed || mailUnavailable || phoneUnavailable;
    const requireCode = Boolean(settings && !settings.firstUser && settings.emailCodeRequired);
    const requirePhoneCode = Boolean(settings && !settings.firstUser && settings.phoneCodeRequired);
    ...
    return (
        <form onSubmit={submit} className="space-y-4">
            ...
            {phoneUnavailable ? (
                <Notice icon={<TriangleAlert className="size-3.5" />} tone="amber">
                    管理员已开启手机验证码验证，但尚未完成短信服务商配置，普通注册暂不可用。
                </Notice>
            ) : null}
            ...
            {requirePhoneCode ? (
                <>
                    <AuthField label="手机号">
                        <Input
                            size="large"
                            prefix={<Phone className="size-4 text-white/35" />}
                            value={phone}
                    ...
                    <AuthField label="手机验证码">
                        <div className="grid grid-cols-[minmax(0,1fr)_116px] gap-2">
                            <Input
                                ...
                                disabled={disabled}
                            />
                            <Button size="large" loading={sendingPhoneCode} disabled={disabled || phoneCountdown > 0} onClick={() => void sendPhoneCode()}>
            ...
            {settings?.linuxdoEnabled || settings?.wechatEnabled || settings?.douyinEnabled ? (
                <>
                    ...
                    <div className="grid gap-2">
                        ...
                        {settings?.wechatEnabled ? (
                            <Button size="large" block icon={<MessageCircle className="size-5 text-[#07c160]" />} href={weChatLoginURL(next)}>
                                使用微信注册 / 登录
                            </Button>
                        ) : null}
                        {settings?.douyinEnabled ? (
                            <Button size="large" block icon={<Music2 className="size-5" />} href={douyinLoginURL(next)}>
                                使用抖音注册 / 登录
                            </Button>
                        ) : null}
                    </div>

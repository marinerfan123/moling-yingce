...
export function AuthScene() {
    ...
    const activeTab = location.pathname === "/register" && registrationEnabled !== false ? "register" : "login";
    const copy = recovery ? authCopy.recovery : activeTab === "register" ? authCopy.register : authCopy.login;
    const registrationSettingsLoading = location.pathname === "/register" && registrationEnabled === null;
    const authTabs = registrationEnabled === false ? [AUTH_TABS[0]] : AUTH_TABS;
    ...
    useEffect(() => {
        ...
        void getAuthSettings()
            .then((settings) => !cancelled && setRegistrationEnabled(settings.registrationEnabled))
            .catch(() => !cancelled && setRegistrationEnabled(true));
        return () => {
            cancelled = true;
        };
    }, []);
    ...
    useEffect(() => {
        if (!recovery && registrationEnabled === false && location.pathname === "/register") {
            navigate({ pathname: "/login", search: location.search }, { replace: true });
        }
    }, [location.pathname, location.search, navigate, recovery, registrationEnabled]);
                                ...
                                <section aria-label={copy.title} className={`flex flex-col ${recovery ? "min-h-[600px]" : activeTab === "login" ? "min-h-[500px]" : "min-h-[620px] sm:min-h-[640px]"}`}>
                                    ...
                                    {!recovery ? (
                                        <div className="px-6 sm:px-8">
                                            <Tabs className="auth-card-tabs" activeKey={activeTab} items={authTabs} onChange={(key) => navigate({ pathname: key === "register" ? "/register" : "/login", search: location.search })} />
                                        </div>
                                    ) : null}
                                    <div key={location.pathname} className="flex-1 px-6 py-6 sm:px-8 sm:py-7">
                                        {registrationSettingsLoading ? <div className="auth-scene-muted py-12 text-center text-sm">正在读取注册设置…</div> : <Outlet />}
                                    </div>
                                </section>
                            </div>
                        </ConfigProvider>

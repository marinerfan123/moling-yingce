...
export function ChannelSettingsPane({ onOpenModels, onOpenRunningHub }: ChannelSettingsPaneProps) {
    ...
    const userChannels = config.channels.filter((channel) => channel.scope !== "system" && channel.transport !== "local-runtime" && channel.id !== "local:dreamina-cli" && channel.allowLocalChannel !== true);
    ...
    const refreshChannelModels = async (channel: ModelChannel) => {
        ...
        try {
            const projectedChannel = { ...channel, allowLocalChannel: false };
                                        ...
                                        <div className="model-editor-panel">
                                            <section className="model-editor-section">
                                                ...
                                                <div className="model-editor-connection-fields grid gap-3 sm:grid-cols-2">
                                                    ...
                                                    <Form.Item label="Base URL" htmlFor={`channel-${channel.id}-base-url`} className="mb-0 sm:col-span-1"><Input id={`channel-${channel.id}-base-url`} inputMode="url" value={channel.baseUrl} placeholder="填写渠道 Base URL" onChange={(event) => updateChannel(channel.id, { baseUrl: event.target.value })} onBlur={(event) => updateChannel(channel.id, { baseUrl: event.target.value.trim().replace(/\/+$/, "") })} /></Form.Item>

...
export function ComfyUIBridgeSettingsPane() {
    ...
    const canUseLocalTools = canUseDesktopLocalTools(user, desktopLocalChannelsEnabled);
    ...
    const reload = async () => {
        if (!userId || !canUseLocalTools) return;
        setLoading(true);
        try {
    ...
    const register = async () => {
        if (!canUseLocalTools) return message.error("本机工具仅管理员可用，且必须由部署配置启用");
    ...
    if (!canUseLocalTools) {
        return <p className="rounded-lg border border-border/60 bg-foreground/[0.03] px-4 py-3 text-sm text-foreground/60">本机工具未由当前部署启用，ComfyUI Bridge 不可用。</p>;
    }
        ...
        <Form layout="vertical" requiredMark={false}>
            <div className="settings-pane-header">
                <div className="min-w-0">
                    ...
                    <p>Bridge 主动连接云端，并访问运行 Bridge 的机器能够连接到的任意 ComfyUI 服务；Bridge 仅由管理员注册和管理，普通用户不会获得本机访问权限。</p>
                </div>

...
import { canUseDesktopLocalTools, isDesktopLocalPluginId } from "@/lib/desktop-local-tools";
...
export default function PluginsPage() {
    ...
    const canUseLocalTools = canUseDesktopLocalTools(user, features.desktopLocalChannelsEnabled);
    ...
    useEffect(() => {
        for (const plugin of builtinPlugins) {
            if (!isDesktopLocalPluginId(plugin.manifest.id) || canUseLocalTools) ensurePlugin(plugin.manifest);
        }
    }, [builtinPlugins, canUseLocalTools, ensurePlugin]);
    ...
    const filteredPlugins = useMemo(() => {
        ...
        return registeredPlugins.filter((plugin) => {
            if (isDesktopLocalPluginId(plugin.manifest.id) && !canUseLocalTools) return false;
    ...
    }, [backendPluginById, canUseLocalTools, categoryFilter, features.systemPluginsVisibleToUsers, installations, pluginStates, registeredPlugins, search, statusFilter, trustFilter, user?.role]);
    ...
    const categoryCounts = useMemo(() => {
        const visiblePlugins = registeredPlugins.filter((plugin) => (isDesktopLocalPluginId(plugin.manifest.id) ? canUseLocalTools : user?.role === "admin" || features.systemPluginsVisibleToUsers || isOfficialApplicationPlugin(plugin.manifest.id)));
        ...
        return counts;
    }, [canUseLocalTools, features.systemPluginsVisibleToUsers, registeredPlugins, user?.role]);
    ...
    const canConfigurePlugin = (plugin: RegisteredPlugin) => Boolean(pluginStates[plugin.manifest.id]?.canConfigure || (canUseLocalTools && isDesktopLocalPluginId(plugin.manifest.id))) && (hasPluginConfiguration(plugin) || plugin.manifest.id === RUNNINGHUB_PLUGIN_ID || plugin.manifest.id === COMFYUI_PLUGIN_ID || plugin.manifest.id === EAGLE_PLUGIN_ID);
                                            ...
                                            <Button
                                                ...
                                                icon={<ExternalLink className="size-4" />}
                                                    onClick={() => {
                                                        ...
                                                        navigate(settingsPlugin.manifest.id === RUNNINGHUB_PLUGIN_ID ? "/settings?section=runninghub" : "/admin/local-tools");
                                                    }}
                                            >
                                                {settingsPlugin.manifest.id === RUNNINGHUB_PLUGIN_ID ? "打开工作流设置" : "打开本机工具"}
                                            </Button>
                                        </div>
                                    ) : (
                                        <div className="plugin-settings-empty">

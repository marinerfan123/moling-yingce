...
type UseCanvasSelectionControllerOptions = {
    ...
    visibleNodesRef: { current: CanvasNodeData[] };
...
export function useCanvasSelectionController({
    ...
    const handleCanvasMouseDown = useCallback((event: ReactPointerEvent<HTMLDivElement>) => {
        ...
        const visibleNodeIds = new Set(visibleNodesRef.current.map((node) => node.id));
        const initialSelection = new Set([...selectedNodeIdsRef.current].filter((nodeId) => visibleNodeIds.has(nodeId)));
        if (!sameStringSet(initialSelection, selectedNodeIdsRef.current)) {
            selectedNodeIdsRef.current = initialSelection;
        ...
        selectionSpatialIndexCacheRef.current.get(visibleNodesRef.current);
        setSelectedConnectionId(null);
    }, [cancelPendingConnectionCreate, onCanvasSelectionStart, screenToCanvas, selectedNodeIdsRef, setSelectedConnectionId, setSelectedNodeIds, visibleNodesRef]);
    ...
    const handleNodeMouseDown = useCallback((event: ReactMouseEvent | ReactPointerEvent, nodeId: string) => {
        ...
        const nextSelected = new Set([...selectedNodeIdsRef.current].filter((selectedId) => visibleNodeIds.has(selectedId)));
    ...
    }, [containerRef, historyPausedRef, onBatchConnectionTarget, onNodeBringToFront, onNodeClick, onNodeInteractionStart, selectedNodeIdsRef, setSelectedConnectionId, setSelectedNodeIds, visibleNodesRef]);
    ...
    const finishNodeDrag = useCallback((clientX?: number, clientY?: number) => {
        ...
        if (dragRef.current.hasMoved) {
            // Authorization can change while the pointer is held down. Only
            // commit nodes that are still present in the current visible set.
            const visibleNodeIds = new Set(visibleNodesRef.current.map((node) => node.id));
            const draggedNodeIds = new Set(dragRef.current.draggedNodeIds.filter((nodeId) => visibleNodeIds.has(nodeId)));
            ...
            const draggedNodes = draggedNodesRef.current.filter((node) => draggedNodeIds.has(node.id));
            const positioned = clientX == null || clientY == null ? nodesRef.current : nodesRef.current.map((node) => {
                ...
                return initial && draggedNodeIds.has(node.id) ? { ...node, position: { x: initial.x + dx, y: initial.y + dy } } : node;
            });
            const targetId = findFrameDropTargetFromIndex(frameDropIndexRef.current, draggedNodes, draggedNodeIds, { x: dx, y: dy });
            ...
            if (clickedNodeId && draggedNodeIds.has(clickedNodeId)) onNodeDragEnd?.(clickedNodeId);
        }
        setFrameDropTargetId(null);
        alignmentContextRef.current = null;
        draggedNodesRef.current = [];
        ...
        if (wasClick && clickedNodeId) {
            const clickedNode = visibleNodesRef.current.find((node) => node.id === clickedNodeId);
            if (clickedNode) onNodeClick(clickedNode);
        }
    }, [historyPausedRef, nodesRef, onLinkedFolderDrop, onNodeClick, onNodeDragEnd, setNodes, viewportRef, visibleNodesRef]);
    ...
    const updateSelectionPreview = useCallback((world: Position, commit: boolean) => {
        ...
        return true;
    }, [containerRef, selectedNodeIdsRef, setSelectedNodeIds, viewportRef, visibleNodesRef]);

...
export function useCanvasViewportController({
    ...
    }, [focusNodesInView, visibleNodesRef]);
    ...
    }, [focusNodesInView, selectedNodeIdsRef, visibleNodesRef]);
    ...
    const focusCanvasImageNode = useCallback((nodeId: string) => {
        const node = visibleNodesRef.current.find((item) => item.id === nodeId && item.type === CanvasNodeType.Image);
        if (!node) return;
    ...
    }, [selectFocusedNode, setDialogNodeId, setToolbarNodeId, size.height, size.width, transitionViewportTo, viewportRef, visibleNodesRef]);
    ...
    }, [selectFocusedNode, setDialogNodeId, size.height, size.width, transitionViewportTo, viewportRef, visibleNodesRef]);

...
import { canUseDesktopLocalTools, filterCanvasProjectForUser } from "@/lib/desktop-local-tools";
...
export default function CanvasPage() {
    ...
    const allowDesktopLocalTools = useUserStore((state) => canUseDesktopLocalTools(state.user, state.features.desktopLocalChannelsEnabled));
    ...
    const projects = useMemo<CanvasLibrarySummary[]>(() => userId
        ...
        : localProjects.map((project) => {
            const visibleProject = filterCanvasProjectForUser(project, allowDesktopLocalTools);
            return { ...visibleProject, nodeCount: visibleProject.nodes.length, previewNodes: visibleProject.nodes.slice(0, 4) };
        }), [allowDesktopLocalTools, libraryQuery.data, localProjects, userId]);
    ...
    const exportSelected = async () => {
        try {
            ...
            await exportCanvasProjects(selected, `${brandName}画布-${selected.length}个画布`, allowDesktopLocalTools);
    ...
    const importCanvas = async (file?: File) => {
        ...
        try {
            ...
            for (const item of data.projects) {
                const projectForImport = filterCanvasProjectForUser(item.project, allowDesktopLocalTools);
                const projectStorageKeys = new Set(collectCanvasStorageKeys(projectForImport));
                const filesForImport = allowDesktopLocalTools ? item.files : item.files.filter((entry) => projectStorageKeys.has(entry.storageKey));
                const totalFiles = filesForImport.length;
                const importedProjectId = importProject({
                    ...projectForImport,
                    title: projectForImport.title || "导入画布",
                    nodes: projectForImport.nodes || [],
                });
                ...
                try {
                    ...
                    const concurrency = 4;
                    let fileIndex = 0;
                    const workers = new Array(Math.min(filesForImport.length, concurrency)).fill(null).map(async () => {
                        while (fileIndex < filesForImport.length) {
                            const current = fileIndex++;
                            const fileItem = filesForImport[current];
                    ...
                    let remappedNodes = (projectForImport.nodes || []).map(remapNodeMedia);
                    let remappedTimeline = projectForImport.timeline
                        ? {
                              ...projectForImport.timeline,
                              clips: projectForImport.timeline.clips.map((clip) => {
                    ...
                    for (let index = 0; index < remappedNodes.length; index += 1) {
                        ...
                        if (!assetId) {
                            const result = await ensureCanvasNodeAsset({ canvasId: importedProjectId, domainProjectId: projectForImport.projectId, node, source: "canvas-upload" });

...
export function useCanvasAssistantVisibility(allowLocalAgent = false) {
    ...
    const openAgent = useCallback((mode?: CanvasAgentMode) => {
        ...
        if (mode) {
            if (mode === "local" && !allowLocalAgent) return;
            setAgentMode(mode);
        }
    ...
    const changeAgentMode = useCallback((mode: CanvasAgentMode) => {
        if (mode === "local" && !allowLocalAgent) {
            setAgentMode("online");
            return;
        }
        setAgentMode(mode);

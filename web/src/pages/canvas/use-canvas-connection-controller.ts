...
import { isDesktopLocalCanvasNode, isDesktopLocalNodeType, isDesktopLocalWorkflowProvider } from "@/lib/desktop-local-tools";
...
function isDesktopLocalNodeId(nodes: CanvasNodeData[], nodeId: string | null | undefined) {
    const node = nodeId ? nodes.find((item) => item.id === nodeId) : undefined;
    return Boolean(node && isDesktopLocalCanvasNode(node));
}

function hasDesktopLocalNodeIds(nodes: CanvasNodeData[], nodeIds: readonly string[]) {
    return nodeIds.some((nodeId) => isDesktopLocalNodeId(nodes, nodeId));
}
...
export function useCanvasConnectionController({
    ...
    const previewBatchConnection = useCallback((sourceNodeIds: string[], targetNodeId: string | null, targetHandleId: string | undefined, targetAnchorRatio: number | undefined, mouseWorld: Position) => {
        if (!allowDesktopLocalTools && (hasDesktopLocalNodeIds(nodesRef.current, sourceNodeIds) || isDesktopLocalNodeId(nodesRef.current, targetNodeId))) {
            updateBatchConnectionPreview({ sourceNodeIds, targetNodeId, targetHandleId, targetAnchorRatio, mouseWorld, status: "invalid" });
            return null;
        }
        ...
        return plan;
    }, [allowDesktopLocalTools, config, connectionsRef, nodesRef, updateBatchConnectionPreview]);
    ...
    const commitBatchConnection = useCallback((sourceNodeIds: string[], targetNodeId: string, targetHandleId?: string, targetAnchorRatio?: number) => {
        if (!allowDesktopLocalTools && (hasDesktopLocalNodeIds(nodesRef.current, sourceNodeIds) || isDesktopLocalNodeId(nodesRef.current, targetNodeId))) {
            message.warning("普通用户不能操作本机节点或本机工作流");
            return { connections: [], connected: [], skipped: [], duplicates: [] };
        }
        ...
        return plan;
    }, [allowDesktopLocalTools, config, connectionsRef, message, nodesRef, setConnections, setContextMenu, setNodes]);
    ...
    const connectNodes = useCallback((current: ConnectionHandle, targetNodeId: string, targetHandleId?: string, targetAnchorRatio?: number) => {
        if (!allowDesktopLocalTools && (isDesktopLocalNodeId(nodesRef.current, current.nodeId) || isDesktopLocalNodeId(nodesRef.current, targetNodeId))) {
            message.warning("普通用户不能操作本机节点或本机工作流");
            return;
        }
    ...
    const createConnectedNode = useCallback(async (type: CanvasNodeType.Image | CanvasNodeType.Text | CanvasNodeType.Script | CanvasNodeType.Video | CanvasNodeType.Audio | CanvasNodeType.Drawing | CanvasNodeType.Config | CanvasNodeType.MediaConversion, pending: PendingConnectionCreate, workflowProvider?: "runninghub" | "comfyui") => {
        const nodeType = type;
        if (isDesktopLocalNodeType(nodeType) && (!allowDesktopLocalTools || !isPluginEffectivelyEnabled(nodeType))) {
        ...
        if (!allowDesktopLocalTools && batchSourceNodes.some(isDesktopLocalCanvasNode)) {
            message.error("普通用户不能操作本机节点或本机工作流");
            closeConnectionCreateMenu();
            setConnecting(null);
            return;
        }
        ...
        closeConnectionCreateMenu();
        setConnecting(null);
    }, [allowDesktopLocalTools, closeConnectionCreateMenu, config, connectionsRef, defaultDrawingEngine, message, nodesRef, projectId, runtimeStatuses, setConnecting, setConnections, setDialogNodeId, setDrawingNodeId, setNodes, setSelectedConnectionId, setSelectedNodeIds, tldrawLicenseKey]);
    ...
    const getConnectionCreateDisabledReason = useCallback((type: CanvasNodeType.Image | CanvasNodeType.Text | CanvasNodeType.Script | CanvasNodeType.Video | CanvasNodeType.Audio | CanvasNodeType.Drawing | CanvasNodeType.Config | CanvasNodeType.MediaConversion, pending: PendingConnectionCreate, workflowProvider?: "runninghub" | "comfyui") => {
        const nodeType = type;
        if (isDesktopLocalNodeType(nodeType) && (!allowDesktopLocalTools || !isPluginEffectivelyEnabled(nodeType))) return "本机工具仅管理员可用或尚未由管理员开放";
        if (nodeType === CanvasNodeType.Config) {
            if (!allowDesktopLocalTools && isDesktopLocalWorkflowProvider(workflowProvider)) return "本机工作流仅管理员可用";
        ...
        if (pending.batchSourceNodeIds?.length) {
            if (!allowDesktopLocalTools && hasDesktopLocalNodeIds(nodesRef.current, pending.batchSourceNodeIds)) return "普通用户不能操作本机节点或本机工作流";
    ...
    }, [allowDesktopLocalTools, config, connectionsRef, nodesRef, runtimeStatuses]);
    ...
    const getConnectionDropTarget = useCallback((clientX: number, clientY: number, current: ConnectionHandle): ConnectionDropTarget => {
        ...
        [...nodesRef.current]
            .filter((node) => (allowDesktopLocalTools || !isDesktopLocalCanvasNode(node)) && !isHiddenBatchChild(node, nodesRef.current) && !isNodeHiddenByCollapsedFrame(node, nodesRef.current) && !isFrameNode(node))
            .reverse()
            .forEach((node) => {
    ...
    }, [allowDesktopLocalTools, config, connectionsRef, nodesRef, screenToCanvas, scriptScrollTopById, viewportRef]);
    ...
    const getBatchConnectionDropTarget = useCallback((clientX: number, clientY: number, sourceNodeIds: string[]): BatchConnectionDropTarget => {
        ...
        return { nodeId: bestNodeId, handleId: bestHandleId, anchorRatio: bestAnchorRatio, isNearNode };
    }, [allowDesktopLocalTools, nodesRef, screenToCanvas, scriptScrollTopById, viewportRef]);
    ...
    const handleConnectStart = useCallback((event: ReactPointerEvent, nodeId: string, handleType: "source" | "target", handleId?: string, anchorRatio?: number) => {
        event.preventDefault();
        event.stopPropagation();
        if (!allowDesktopLocalTools && isDesktopLocalNodeId(nodesRef.current, nodeId)) return;
        ...
        setConnectionTargetAnchorRatio(undefined);
        setSelectedConnectionId(null);
    }, [allowDesktopLocalTools, clearBatchConnection, closeConnectionCreateMenu, commitBatchConnection, nodesRef, screenToCanvas, setConnecting, setSelectedConnectionId]);

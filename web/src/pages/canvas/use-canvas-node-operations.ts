...
import { isDesktopLocalCanvasNode, isDesktopLocalNodeType, isDesktopLocalWorkflowProvider, visibleCanvasNodesForUser } from "@/lib/desktop-local-tools";
...
type UseCanvasNodeOperationsOptions = {
    ...
    allowDesktopLocalTools: boolean;
...
export function useCanvasNodeOperations({
    ...
    const selectNodes = useCallback((ids: Set<string>) => {
        const next = allowDesktopLocalTools
            ? ids
    ...
    }, [allowDesktopLocalTools, nodesRef, selectedNodeIdsRef, setSelectedConnectionId, setSelectedNodeIds]);

    const createNode = useCallback((type: CanvasNodeTypeId, position?: Position, workflowProvider?: "runninghub" | "comfyui") => {
        if (isDesktopLocalNodeType(type) && (!allowDesktopLocalTools || !isPluginEffectivelyEnabled(type))) {
            message.error("本机工具仅管理员可用或尚未由管理员开放");
            return;
        }
        ...
        const selectedWorkflowProvider = type === CanvasNodeType.Config
            ...
            : undefined;
        if (!allowDesktopLocalTools && isDesktopLocalWorkflowProvider(selectedWorkflowProvider)) {
            message.error("本机工作流仅管理员可用");
            return;
        }
    ...
    }, [allowDesktopLocalTools, commitNodes, defaultDrawingEngine, effectiveConfig.comfyBridge.enabled, effectiveConfig.comfyBridge.workflows.length, effectiveConfig.runningHub.enabled, effectiveConfig.runningHub.workflows.length, getCanvasCenter, message, nodesRef, runtimeStatuses, selectNodes, setDialogNodeId, tldrawLicenseKey]);
    ...
    const arrangeSelectedNodes = useCallback((mode: "row" | "column" | "grid" | "flow") => {
        const selected = visibleCanvasNodesForUser(nodesRef.current, allowDesktopLocalTools).filter((node) => selectedNodeIdsRef.current.has(node.id) && !node.metadata?.locked && !isFrameNode(node));
    ...
    }, [allowDesktopLocalTools, commitNodes, connectionsRef, message, nodesRef, selectedNodeIdsRef]);

    const autoArrangeCanvasNodes = useCallback(() => {
        const currentNodes = nodesRef.current;
        const operableNodes = visibleCanvasNodesForUser(currentNodes, allowDesktopLocalTools);
        const selectedIds = selectedNodeIdsRef.current;
        const hasSelection = selectedIds.size > 0;
        const candidates = operableNodes.filter((node) => {
    ...
    const spreadSelectedNodes = useCallback(() => {
        const currentNodes = nodesRef.current;
        const selected = visibleCanvasNodesForUser(currentNodes, allowDesktopLocalTools).filter((node) => selectedNodeIdsRef.current.has(node.id) && !node.metadata?.locked && !isFrameNode(node) && !isHiddenBatchChild(node, currentNodes));
    ...
    }, [allowDesktopLocalTools, commitNodes, message, nodesRef, selectedNodeIdsRef]);
    ...
    const createStoryboardGroup = useCallback(() => {
        const images = visibleCanvasNodesForUser(nodesRef.current, allowDesktopLocalTools)
    ...
    }, [allowDesktopLocalTools, commitNodes, message, nodesRef, selectNodes, selectedNodeIdsRef]);

    const createReferenceGroup = useCallback(() => {
        const media = visibleCanvasNodesForUser(nodesRef.current, allowDesktopLocalTools)
    ...
    const toggleNodeLocked = useCallback((nodeId: string) => {
        ...
        if (!target || (!allowDesktopLocalTools && isDesktopLocalCanvasNode(target))) return;
    ...
    }, [allowDesktopLocalTools, commitNodes, message, nodesRef]);

    const deleteNodes = useCallback((ids: Set<string>) => {
        const deletableIds = allowDesktopLocalTools
            ? ids
            : new Set([...ids].filter((id) => {
                const node = nodesRef.current.find((item) => item.id === id);
                return node && !isDesktopLocalCanvasNode(node);
        ...
        const result = removeCanvasNodes(previousNodes, deletableIds);
    ...
    }, [allowDesktopLocalTools, commitConnections, commitNodes, connectionsRef, nodesRef, onNodesDeleted, selectNodes]);

    const deleteConnection = useCallback((connectionId: string) => {
        const previousNodes = nodesRef.current;
        const previousConnections = connectionsRef.current;
        if (!allowDesktopLocalTools && previousConnections.some((connection) => connection.id === connectionId && (previousNodes.find((node) => node.id === connection.fromNodeId && isDesktopLocalCanvasNode(node)) || previousNodes.find((node) => node.id === connection.toNodeId && isDesktopLocalCanvasNode(node))))) return;
    ...
    }, [allowDesktopLocalTools, commitConnections, commitNodes, connectionsRef, nodesRef, setContextMenu, setSelectedConnectionId]);

    const duplicateNode = useCallback((nodeId: string, duplicateMode: "variant" | "copy" = "variant") => {
        const source = nodesRef.current.find((node) => node.id === nodeId);
        if (!source || (!allowDesktopLocalTools && isDesktopLocalCanvasNode(source))) return;
    ...
    }, [allowDesktopLocalTools, cloneDrawingForNode, commitConnections, commitNodes, connectionsRef, nodesRef, selectNodes, setDialogNodeId]);
    ...
    const setPrimaryVersion = useCallback((nodeId: string) => {
        ...
        commitNodes(nodesRef.current.map((node) => {
            if (!allowDesktopLocalTools && isDesktopLocalCanvasNode(node)) return node;
            return (node.metadata?.versionOfNodeId || node.id) === rootId ? { ...node, metadata: { ...node.metadata, versionPrimary: node.id === nodeId } } : node;
        }));
    ...
    const copyNodesToClipboard = useCallback((targetIds: Set<string>) => {
        const allowedTargetIds = allowDesktopLocalTools
            ? targetIds
            : new Set([...targetIds].filter((id) => {
                const node = nodesRef.current.find((item) => item.id === id);
                return node && !isDesktopLocalCanvasNode(node);
            }));
        if (!allowedTargetIds.size) return;
        const copyIds = new Set(allowedTargetIds);
        nodesRef.current.forEach((node) => {
            if (allowedTargetIds.has(node.id) && isFrameNode(node)) getFrameChildIds(node.id, nodesRef.current).forEach((childId) => {
                const child = nodesRef.current.find((item) => item.id === childId);
                if (allowDesktopLocalTools || (child && !isDesktopLocalCanvasNode(child))) copyIds.add(childId);
            });
        });
    ...
    }, [allowDesktopLocalTools, connectionsRef, nodesRef]);
    ...
    const pasteCopiedNodes = useCallback((position?: Position) => {
        ...
        const clipboardNodes = visibleCanvasNodesForUser(clipboard.nodes, allowDesktopLocalTools);
        if (!clipboardNodes.length) return false;
        const clipboardNodeIds = new Set(clipboardNodes.map((node) => node.id));
        const clipboardConnections = clipboard.connections.filter((connection) => clipboardNodeIds.has(connection.fromNodeId) && clipboardNodeIds.has(connection.toNodeId));
        const center = position || getCanvasCenter();
        const bounds = clipboardNodes.reduce((current, node) => ({
        ...
        const idMap = new Map(clipboardNodes.map((node, index) => [node.id, `${node.type}-${Date.now()}-${index}-${Math.random().toString(36).slice(2, 7)}`]));
        const copiedSourceIds = new Set(clipboardNodes.map((node) => node.id));
        const reservedTitles = new Set(nodesRef.current.map((node) => node.title));
        const nextNodes = clipboardNodes.map((node) => {
            const metadata = isolateCopiedNodeMetadata(node, idMap);
        ...
        const nextConnections = clipboardConnections.flatMap((connection, index) => {
        ...
        const sourceByTargetId = new Map(clipboardNodes.map((sourceNode) => [idMap.get(sourceNode.id), sourceNode]));
        ...
        return true;
    }, [allowDesktopLocalTools, cloneDrawingForNode, commitConnections, commitNodes, connectionsRef, getCanvasCenter, nodesRef, selectNodes, setContextMenu, setDialogNodeId]);
    ...
    const restoreCopiedNodesFromText = useCallback((value: string) => {
        ...
        try {
            ...
            const nodes = visibleCanvasNodesForUser(parsed.nodes, allowDesktopLocalTools);
            const nodeIds = new Set(nodes.map((node) => node.id));
            clipboardRef.current = {
                nodes,
                connections: (parsed.connections || []).filter((connection) => nodeIds.has(connection.fromNodeId) && nodeIds.has(connection.toNodeId)),
            };

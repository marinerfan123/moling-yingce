...
import { visibleCanvasConnectionsForUser, visibleCanvasNodesForUser } from "@/lib/desktop-local-tools";
...
export function useCanvasRenderModel({
    ...
    const canvasNodes = useMemo(() => visibleCanvasNodesForUser(nodes, allowDesktopLocalTools), [allowDesktopLocalTools, nodes]);
    const canvasConnections = useMemo(() => visibleCanvasConnectionsForUser(nodes, connections, allowDesktopLocalTools), [allowDesktopLocalTools, connections, nodes]);
    const reduceMediaEffects = useMemo(() => shouldReduceCanvasMediaEffects(mediaPerformanceMode, canvasNodes), [canvasNodes, mediaPerformanceMode]);
    ...
    }, [canvasNodes, collapsingBatchIds, nodeById]);
    ...
    const nodeSpatialIndex = useMemo(() => {
        ...
        const geometryUnchanged =
            previous &&
            previous.source.length === canvasNodes.length &&
            canvasNodes.every((node, index) => {
        ...
        const index = buildCanvasSpatialIndex(canvasNodes.map((node) => ({ id: node.id, bounds: canvasNodeBounds(node), value: node.id })));
        nodeSpatialIndexRef.current = { source: canvasNodes, index };
        return index;
    }, [canvasNodes]);
    ...
    const semanticNodesRef = useRef(canvasNodes);
    const semanticNodes = useMemo(() => {
        const previous = semanticNodesRef.current;
        const positionOnlyChange = previous.length === canvasNodes.length && canvasNodes.every((node, index) => sameNodeSemanticData(node, previous[index]));
        if (!positionOnlyChange) semanticNodesRef.current = canvasNodes;
        return semanticNodesRef.current;
    }, [canvasNodes]);
    const versionCompareNodes = useMemo(() => {
        if (!versionCompareRootId) return [];
        return canvasNodes.filter((node) => (node.metadata?.versionOfNodeId || node.id) === versionCompareRootId).sort((a, b) => (a.metadata?.versionLabel || "").localeCompare(b.metadata?.versionLabel || ""));
    }, [canvasNodes, versionCompareRootId]);
    ...
    }, [activeNodeId, canvasConnections]);
    ...
    }, [canvasConnections, collapsedBatchChildIds, nodeById]);
    ...
    const configInputsById = useMemo(() => {
        ...
        configNodeIds.forEach((nodeId) => map.set(nodeId, buildNodeGenerationInputs(nodeId, semanticNodes, canvasConnections)));
        return map;
    }, [canvasConnections, dialogNodeId, nodeById, selectedNodeIds, semanticNodes, visibleNodes]);
    ...
    const canvasResourceReferences = useMemo(
        () => buildCanvasResourceReferences(semanticNodes, canvasConnections, dialogNodeId || activeNodeId, resourceReferenceTargetNodes),
        [activeNodeId, canvasConnections, dialogNodeId, resourceReferenceTargetNodes, semanticNodes],
    );
    ...
    const mentionReferencesByNodeId = useMemo(() => {
        const map = buildCanvasNodeMentionReferenceMap(semanticNodes, canvasConnections, visibleNodes);
        ...
        return map;
    }, [canvasConnections, semanticNodes, skillMentionReferences, visibleNodes]);

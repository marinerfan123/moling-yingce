...
import { canUseDesktopLocalTools, isDesktopLocalCanvasNode } from "@/lib/desktop-local-tools";
        ...
        (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string, options?: CanvasNodeGenerationOptions) =>
            runCanvasGenerationSubmissionOnce(
                ...
                async () => {
                    ...
                    if (sourceNode && isDesktopLocalCanvasNode(sourceNode) && !canUseDesktopLocalTools(userState.user, userState.features.desktopLocalChannelsEnabled)) {
                        message.error("本机工具仅管理员可用");
                        return;
                    }

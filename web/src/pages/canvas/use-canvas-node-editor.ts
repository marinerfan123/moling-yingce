...
import { isDesktopLocalCanvasNode } from "@/lib/desktop-local-tools";
...
export function useCanvasNodeEditor({
    ...
    const canOperateNode = useCallback((nodeId: string) => {
        const node = nodesRef.current.find((item) => item.id === nodeId);
        return Boolean(node && (allowDesktopLocalTools || !isDesktopLocalCanvasNode(node)));
    }, [allowDesktopLocalTools, nodesRef]);
    ...
    const handleNodeResize = useCallback((nodeId: string, width: number, height: number, position?: Position) => {
        if (!canOperateNode(nodeId)) return;
        setNodes((current) => {
            let changed = false;
            const next = current.map((node) => {
                if (node.id !== nodeId || node.metadata?.locked || (!allowDesktopLocalTools && isDesktopLocalCanvasNode(node))) return node;
    ...
    }, [allowDesktopLocalTools, canOperateNode, setNodes]);
    ...
    }, [canOperateNode, nodesRef, setDialogNodeId, setHoveredNodeId, setNodes, setSelectedConnectionId, setSelectedNodeIds, setToolbarNodeId]);

    const handleNodeTitleChange = useCallback((nodeId: string, title: string) => {
        if (!canOperateNode(nodeId)) return;
        setNodes((current) => current.map((node) => (node.id === nodeId ? { ...node, title } : node)));
    }, [canOperateNode, setNodes]);
    ...
    }, [canOperateNode, nodesRef, setNodes]);

    const setBatchPrimary = useCallback((child: CanvasNodeData) => {
        if (!canOperateNode(child.id)) return;
    ...
    }, [canvasId, canOperateNode, domainProjectId, message, nodesRef, queryClient, setNodes]);

    const downloadNodeImage = useCallback((node: CanvasNodeData) => {
        if (!allowDesktopLocalTools && isDesktopLocalCanvasNode(node)) return;
        if ((node.type !== CanvasNodeType.Image && node.type !== CanvasNodeType.Video && node.type !== CanvasNodeType.Audio) || !node.metadata?.content) return;
        saveAs(node.metadata.content, buildCanvasMediaDownloadFileName(canvasTitle, node));
    }, [allowDesktopLocalTools, canvasTitle]);

    const saveNodeAsset = useCallback(async (node: CanvasNodeData) => {
        if (!allowDesktopLocalTools && isDesktopLocalCanvasNode(node)) return message.error("普通用户不能操作本机节点");
    ...
    }, [allowDesktopLocalTools, canvasId, domainProjectId, message, queryClient, setNodes]);

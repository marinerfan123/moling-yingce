...
import { isDesktopLocalCanvasNode, visibleCanvasConnectionsForUser, visibleCanvasNodesForUser } from "@/lib/desktop-local-tools";
...
export function useCanvasAgentOperations({
    ...
    const snapshot = useMemo<CanvasAgentSnapshot>(() => {
        const visibleNodes = visibleCanvasNodesForUser(nodes, allowDesktopLocalTools);
        ...
        return {
            ...
            title: projectTitle,
            nodes: visibleNodes,
            connections: visibleCanvasConnectionsForUser(nodes, connections, allowDesktopLocalTools),
            selectedNodeIds: Array.from(selectedNodeIds).filter((nodeId) => visibleNodeIds.has(nodeId)),
            viewport,
        };
    }, [allowDesktopLocalTools, connections, domainProjectId, nodes, projectId, projectTitle, selectedNodeIds, viewport]);
    ...
    const applyOps = useCallback(
        async (ops?: CanvasAgentOp[], generationContext?: CanvasAgentGenerationContext) => {
            ...
            const before: CanvasAgentSnapshot = { projectId, domainProjectId, title: projectTitle, nodes: nodesRef.current, connections: connectionsRef.current, selectedNodeIds: Array.from(selectedNodeIdsRef.current), viewport: viewportRef.current };
            const beforeVisibleNodes = visibleCanvasNodesForUser(before.nodes, allowDesktopLocalTools);
            const beforeVisibleNodeIds = new Set(beforeVisibleNodes.map((node) => node.id));
            const beforeForAgent: CanvasAgentSnapshot = {
                ...before,
                nodes: beforeVisibleNodes,
                connections: visibleCanvasConnectionsForUser(before.nodes, before.connections, allowDesktopLocalTools),
                selectedNodeIds: before.selectedNodeIds.filter((nodeId) => beforeVisibleNodeIds.has(nodeId)),
            };
            if (!allowDesktopLocalTools) assertDesktopLocalOpsAllowed(safeOps, before.nodes, before.connections);
            const generationOps = safeOps.filter((op): op is Extract<CanvasAgentOp, { type: "run_generation" }> => op.type === "run_generation" && Boolean(op.nodeId));
            const next = applyCanvasAgentOps(
                beforeForAgent,
                safeOps.filter((op) => op.type !== "run_generation"),
            );
            const nextProject = allowDesktopLocalTools ? next : mergeCanvasAgentViewIntoProject(before, next);
            const beforeNodeIds = new Set(before.nodes.map((node) => node.id));
            const addedNodeIds = nextProject.nodes.filter((node) => !beforeNodeIds.has(node.id)).map((node) => node.id);
            const addedNodeIdSet = new Set(addedNodeIds);
            const focusNodeIds = nextProject.nodes.filter((node) => addedNodeIdSet.has(node.id) && (!node.parentId || !addedNodeIdSet.has(node.parentId))).map((node) => node.id);
            const affectedNodeIds = focusNodeIds.length ? focusNodeIds : agentAffectedNodeIds(safeOps, nextProject.nodes);
            const nextSelectedNodeIds = focusNodeIds.length ? focusNodeIds : nextProject.selectedNodeIds;
            nodesRef.current = nextProject.nodes;
            connectionsRef.current = nextProject.connections;
            selectedNodeIdsRef.current = new Set(nextSelectedNodeIds);
            viewportRef.current = nextProject.viewport;
            setNodes(nextProject.nodes);
            setConnections(nextProject.connections);
            ...
            if (safeOps.length) {
                ...
                undoStackRef.current = [...undoStackRef.current, { snapshot: before, afterNodes: nextProject.nodes, afterConnections: nextProject.connections, change }].slice(-10);
            ...
            return { ...nextProject, nodes: nodesRef.current, connections: connectionsRef.current, projectId, title: projectTitle, selectedNodeIds: nextSelectedNodeIds };
        },
        [allowDesktopLocalTools, connectionsRef, domainProjectId, focusSelection, generateNodeRef, nodesRef, projectId, projectTitle, selectedNodeIdsRef, setConnections, setContextMenu, setNodes, setSelectedConnectionId, setSelectedNodeIds, setViewport, viewportRef],
    );
...
function assertDesktopLocalOpsAllowed(ops: CanvasAgentOp[], nodes: CanvasNodeData[], connections: CanvasConnection[]) {
    const localNodeIds = new Set(nodes.filter(isDesktopLocalCanvasNode).map((node) => node.id));
    const localConnectionIds = new Set(connections.filter((connection) => localNodeIds.has(connection.fromNodeId) || localNodeIds.has(connection.toNodeId)).map((connection) => connection.id));
    const reject = () => {
        throw new Error("普通用户不能操作本机节点或本机工作流");
    };

    for (const op of ops) {
        if (op.type === "add_node" && isDesktopLocalCanvasNode({ type: op.nodeType || "text", metadata: op.metadata })) reject();
        if (op.type === "update_node") {
            if (localNodeIds.has(op.id)) reject();
            const current = nodes.find((node) => node.id === op.id);
            const next = current
                ? { ...current, ...op.patch, metadata: { ...current.metadata, ...op.patch?.metadata, ...op.metadata } }
                : undefined;
            if (next && isDesktopLocalCanvasNode(next)) reject();
        }
        if (op.type === "run_generation" && localNodeIds.has(op.nodeId)) reject();
        if (op.type === "connect_nodes" && (localNodeIds.has(op.fromNodeId) || localNodeIds.has(op.toNodeId))) reject();
        if (op.type === "select_nodes" && op.ids.some((id) => localNodeIds.has(id))) reject();
        if (op.type === "delete_node") {
            const ids = op.ids || (op.id ? [op.id] : op.nodeType ? nodes.filter((node) => node.type === op.nodeType).map((node) => node.id) : []);
            if (ids.some((id) => localNodeIds.has(id))) reject();
        }
        if (op.type === "delete_connections" && (op.all || (op.ids || (op.id ? [op.id] : [])).some((id) => localConnectionIds.has(id)))) reject();
    }
}

function mergeCanvasAgentViewIntoProject(before: CanvasAgentSnapshot, nextVisible: CanvasAgentSnapshot): CanvasAgentSnapshot {
    const hiddenNodes = before.nodes.filter(isDesktopLocalCanvasNode);
    const hiddenNodeIds = new Set(hiddenNodes.map((node) => node.id));
    const nextById = new Map(nextVisible.nodes.map((node) => [node.id, node]));
    const nodes = before.nodes.flatMap((node) => {
        if (hiddenNodeIds.has(node.id)) return [node];
        const next = nextById.get(node.id);
        return next ? [next] : [];
    });
    const beforeNodeIds = new Set(before.nodes.map((node) => node.id));
    nodes.push(...nextVisible.nodes.filter((node) => !beforeNodeIds.has(node.id)));
    const nextNodeIds = new Set(nodes.map((node) => node.id));
    const hiddenConnections = before.connections.filter((connection) =>
        (hiddenNodeIds.has(connection.fromNodeId) || hiddenNodeIds.has(connection.toNodeId))
        && nextNodeIds.has(connection.fromNodeId)
        && nextNodeIds.has(connection.toNodeId),
    );
    return {
        ...nextVisible,
        nodes,
        connections: [...nextVisible.connections, ...hiddenConnections],
        selectedNodeIds: nextVisible.selectedNodeIds.filter((nodeId) => nextNodeIds.has(nodeId)),
    };
}

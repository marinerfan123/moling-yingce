...
import { canUseDesktopLocalTools, clearCanvasStateForUser, isDesktopLocalCanvasNode, visibleCanvasConnectionsForUser, visibleCanvasNodesForUser } from "@/lib/desktop-local-tools";
...
function InfiniteCanvasPage() {
    ...
    const allowLocalAgent = useUserStore((state) => canUseDesktopLocalTools(state.user, state.features.desktopLocalChannelsEnabled));
    ...
    const canvasNodes = useMemo(() => visibleCanvasNodesForUser(nodes, allowLocalAgent), [allowLocalAgent, nodes]);
    const canvasConnections = useMemo(() => visibleCanvasConnectionsForUser(nodes, connections, allowLocalAgent), [allowLocalAgent, connections, nodes]);
    ...
    const codexAutoConnect = allowLocalAgent && shouldAutoConnectCanvasRuntime(searchParams);
    const codexCompactAgent = allowLocalAgent && codexAutoConnect && readLocalRuntimeBootstrapState().legacyDeepLinkRejected;
    ...
    useEffect(() => {
        if (allowLocalAgent) return;
        const visibleNodeIds = new Set(canvasNodes.map((node) => node.id));
        setSelectedNodeIds((current) => {
            const next = new Set([...current].filter((nodeId) => visibleNodeIds.has(nodeId)));
            selectedNodeIdsRef.current = next;
            return next.size === current.size ? current : next;
        });
        setSelectedConnectionId((current) => current && canvasConnections.some((connection) => connection.id === current) ? current : null);
        setHoveredNodeId((current) => current && visibleNodeIds.has(current) ? current : null);
        setToolbarNodeId((current) => current && visibleNodeIds.has(current) ? current : null);
        setDialogNodeId((current) => current && visibleNodeIds.has(current) ? current : null);
        setPortraitClearanceNodeId(null);
        setContextMenu((current) => {
            if (!current) return current;
            if (current.type === "node" && !visibleNodeIds.has(current.nodeId)) return null;
            if (current.type === "connection" && !canvasConnections.some((connection) => connection.id === current.connectionId)) return null;
            return current;
        });
    }, [allowLocalAgent, canvasConnections, canvasNodes]);
    ...
    const { agentMode, assistantClosing, assistantMounted, assistantOpen, closeAgent, openAgent, setAgentMode } = useCanvasAssistantVisibility(allowLocalAgent);
    ...
    const canvasContext = useMemo(() => summarizeCanvasContext(canvasNodes, selectedNodeIds, linkedProjectQuery.data?.units), [canvasNodes, linkedProjectQuery.data?.units, selectedNodeIds]);
    ...
    const nodeGraphContext = useMemo<CanvasNodeGraphContextValue>(() => ({ getUpstreamNodes: (nodeId: string) => getContextResourceNodes(nodeId, canvasNodes, canvasConnections) }), [canvasConnections, canvasNodes]);
    ...
    }, [activeChatId, allowLocalAgent, canvasNodes, chatSessions, nodes, connections, selectedNodeIds, viewport]);
    ...
    const {
        ...
        size,
        viewportRef,
        visibleNodesRef: canvasNodesRef,
        selectedNodeIdsRef,
        setViewport,
        setSelectedNodeIds,
        setSelectedConnectionId,
    ...
    const {
        ...
        nodesRef,
        connectionsRef,
        allowDesktopLocalTools: allowLocalAgent,
        selectedNodeIdsRef,
        getCanvasCenter,
        setNodes,
        setConnections,
    ...
    const handleReplaceNodeReference = useCallback((
        ...
        if (!allowLocalAgent && [targetNodeId, sourceNodeId, oldReference.nodeId, configNodeId].some((id) => {
            const node = id ? previousNodes.find((item) => item.id === id) : undefined;
            return node ? isDesktopLocalCanvasNode(node) : false;
        })) return;
    ...
    }, [allowLocalAgent, connectionsRef, message, nodesRef, setConnections, setNodes]);
    ...
    const handleReplaceNodeReferenceFiles = useCallback((
        ...
        const referenceNode = nodesRef.current.find((node) => node.id === oldReference.nodeId);
        if (!allowLocalAgent && referenceNode && isDesktopLocalCanvasNode(referenceNode)) return;
        void replaceNodeMedia(oldReference.nodeId, file).then((success: boolean) => {
            if (success) {
                message.success("参考图片已替换");
            }
        });
    }, [allowLocalAgent, message, nodesRef, replaceNodeMedia]);
    ...
    useEffect(() => {
        if (allowLocalAgent || !pendingConnectionCreate) return;
        const sourceNodeId = pendingConnectionCreate.connection.nodeId;
        if (!canvasNodes.some((node) => node.id === sourceNodeId)) cancelPendingConnectionCreate();
    }, [allowLocalAgent, cancelPendingConnectionCreate, canvasNodes, pendingConnectionCreate]);

    const batchSourceNodeIds = useMemo(() => canvasNodes
        .filter((node) => selectedNodeIds.has(node.id) && !batchSourceRestriction(node))
        .map((node) => node.id), [canvasNodes, selectedNodeIds]);
    ...
    usePortraitClearanceCoordinator({ nodes: canvasNodes, onUpdateState: handlePortraitClearanceStateUpdate });
    ...
    const handleRemoveNodeReference = useCallback((targetNodeId: string, reference: CanvasResourceReference) => {
        ...
        if (!allowLocalAgent && [targetNodeId, referenceNodeId, configNodeId].some((id) => {
            const node = id ? previousNodes.find((item) => item.id === id) : undefined;
            return node ? isDesktopLocalCanvasNode(node) : false;
        })) return;
    ...
    }, [allowLocalAgent, connectionsRef, nodesRef, setConnections, setNodes, setSelectedConnectionId]);

    const handleReorderNodeReferences = useCallback((targetNodeId: string, orderedNodeIds: string[]) => {
        const previousNodes = nodesRef.current;
        const previousConnections = connectionsRef.current;
        if (!allowLocalAgent && [targetNodeId, ...orderedNodeIds].some((id) => {
            const node = previousNodes.find((item) => item.id === id);
            return node ? isDesktopLocalCanvasNode(node) : false;
        })) return;
        ...
        connectionsRef.current = nextConnections;
        setConnections(nextConnections);
    }, [allowLocalAgent, connectionsRef, nodesRef, setConnections, setNodes]);
    ...
    const updateNodeFromContent = useCallback((nodeId: string, update: (node: CanvasNodeData) => CanvasNodeData) => {
        ...
        if (!currentNode || (!allowLocalAgent && isDesktopLocalCanvasNode(currentNode))) return;
        setNodes((current) => {
            const target = current.find((node) => node.id === nodeId);
            if (!target || (!allowLocalAgent && isDesktopLocalCanvasNode(target))) return current;
            const next = updateCanvasNode(current, nodeId, update);
            nodesRef.current = next;
            return next;
        });
    }, [allowLocalAgent, nodesRef, setNodes]);
    ...
    const updateMediaNodeFromContent = useCallback((nodeId: string, update: (node: CanvasNodeData) => CanvasNodeData) => {
        ...
        mediaUpdateTimerRef.current = setTimeout(() => {
            ...
            setNodes((current) => {
                const safeUpdates = allowLocalAgent
                    ? updates
                    : new Map([...updates].filter(([nodeId]) => {
                          const node = current.find((item) => item.id === nodeId);
                          return node && !isDesktopLocalCanvasNode(node);
                      }));
                const next = safeUpdates.size ? updateCanvasNodes(current, safeUpdates) : current;
                nodesRef.current = next;
                return next;
            });
        }, 120);
    ...
    const clearCanvas = useCallback(() => {
        const previousNodes = nodesRef.current;
        const previousConnections = connectionsRef.current;
        const drawingIds = previousNodes
            .filter((node) => allowLocalAgent || canvasNodesRef.current.some((visibleNode) => visibleNode.id === node.id))
            .flatMap((node) => (node.type === CanvasNodeType.Drawing && node.metadata?.drawingId ? [node.metadata.drawingId] : []));
        ...
        const { nodes: nextNodes, connections: nextConnections } = clearCanvasStateForUser(previousNodes, previousConnections, allowLocalAgent);
        nodesRef.current = nextNodes;
        connectionsRef.current = nextConnections;
        setNodes(nextNodes);
        setConnections(nextConnections);
        ...
        clearCanvasFiles();
    }, [allowLocalAgent, canvasNodesRef, clearCanvasFiles, connectionsRef, deselectCanvas, message, nodesRef, projectId, setEmotionNodeId]);
    ...
    const renderCanvasNodeContent = useCallback(
        (contentNode: CanvasNodeData) => {
            ...
            if (contentNode.type === CanvasNodeType.Script) {
                const pipeline = deriveStoryboardPipelineProgress(contentNode, canvasNodes, canvasConnections);
                return (
                ...
                <CanvasOverlayLayerProvider>
                        ...
                        {assistantMounted ? (
                                ...
                                {(resizing) => (
                                    <CanvasAssistantPanel
                                        ...
                                        allowLocalAgent={allowLocalAgent}
                    ...
                    {pendingConnectionCreate ? (
                        <CanvasConnectionCreateMenu
                            ...
                            canCreateMediaConversion={allowLocalAgent}
                    ...
                    <CanvasNodeToolbar
                        ...
                        onCreateConversion={allowLocalAgent ? createConversionFromSource : undefined}
                    ...
                    {isMiniMapOpen && !focusMode ? <Minimap nodes={canvasNodes} viewport={viewport} viewportSize={size} canvasContainerRef={containerRef} onViewportPreviewChange={previewViewport} onViewportChange={handleViewportChange} /> : null}
                    ...
                    {directorNodeId && activeDirectorScene ? (
                        ...
                        >
                            <CanvasDirectorWorkbench
                                ...
                                imageNodes={canvasNodes.filter((node) => node.type === CanvasNodeType.Image && Boolean(node.metadata?.content))}

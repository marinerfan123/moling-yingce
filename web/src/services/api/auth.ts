...
export function weChatLoginURL(next: string) {
    return socialLoginURL("wechat", next);
}

export function douyinLoginURL(next: string) {
    return socialLoginURL("douyin", next);
}

function socialLoginURL(provider: "wechat" | "douyin", next: string) {
    const base = String(apiBaseURL).replace(/\/$/, "");
    return `${base}/auth/${provider}/start?next=${encodeURIComponent(next)}`;
}
...
export function sendRegistrationPhoneCode(phone: string) {
    return http.post<{ sent: boolean }>("/auth/phone-code", { phone });
}
...
export function register(input: { username: string; email?: string; emailCode?: string; phone?: string; phoneCode?: string; displayName?: string; password: string }) {
...
export type AdminChannelExport = {
    version: number;
    exportedAt: string;
    channels: Array<Record<string, unknown>>;
};

export async function exportAdminChannels() {
    const snapshot = await http.get<AdminChannelExport>("/admin/channels/export");
    return new Blob([JSON.stringify(snapshot, null, 2)], { type: "application/json;charset=utf-8" });
}

export function importAdminChannels(input: AdminChannelExport) {
    return http.post<{ importedChannels: number; importedModels: number }>("/admin/channels/import", input);
}

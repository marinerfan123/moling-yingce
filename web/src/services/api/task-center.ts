...
import { isDesktopLocalGenerationTask } from "@/lib/desktop-local-tools";
...
type GenerationTaskListOptions = { projectId?: string; activeOnly?: boolean; includeLocal?: boolean };
...
export async function listGenerationTasks(limit = 30, options?: GenerationTaskListOptions, dependencies: GenerationTaskListDependencies = defaultGenerationTaskListDependencies, signal?: AbortSignal) {
    ...
    const backendPromise = collectGenerationTaskPages(backendPageReader, baseRequest, boundedLimit, signal);
    const localPromise = options?.includeLocal === true
        ? collectGenerationTaskPages(
        ...
        : Promise.resolve<LocalDreaminaGenerationTask[]>([]);
    const [backendTasks, localTasks] = await Promise.all([backendPromise, localPromise]);
    if (signal?.aborted) throw new DOMException("Aborted", "AbortError");
    return [...backendTasks, ...localTasks.map((task) => projectLocalDreaminaTask(task))]
        .filter((task) => options?.includeLocal === true || !isDesktopLocalGenerationTask(task))

...
import { CONFIG_STORE_KEY, PUBLIC_MODEL_CATALOG_ID, defaultConfig, effectiveConfigForCustomChannels, normalizeConfigSnapshot, useConfigStore, type ModelCapability, type ModelChannel } from "@/stores/use-config-store";
...
export async function applyUserSession(payload: AuthSessionPayload) {
    ...
    try {
		...
		clearCustomChannelConfigForUser(payload.user);
...
export async function refreshFeatureAvailability() {
    ...
    useUserStore.getState().setFeatures(payload.features);
	clearCustomChannelConfigForUser(useUserStore.getState().user);
    return payload.features;
}

function clearCustomChannelConfigForUser(user: Pick<NonNullable<AuthSessionPayload["user"]>, "role" | "status"> | null) {
    const features = useUserStore.getState().features;
    if (canUseCustomChannels(user, features.customChannelsEnabled)) return;
    const currentConfig = useConfigStore.getState().config;
    useConfigStore.getState().replaceConfig(effectiveConfigForCustomChannels(currentConfig, false));
}

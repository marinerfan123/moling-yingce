...
import { filterCanvasProjectForUser } from "@/lib/desktop-local-tools";

export async function exportCanvasProjects(projects: CanvasProject[], fileName = "画布", allowDesktopLocalTools = false) {
    const zipFiles: { name: string; data: BlobPart }[] = [];
    const visibleProjects = projects.map((project) => filterCanvasProjectForUser(project, allowDesktopLocalTools));
    const exportedProjects = await Promise.all(
        visibleProjects.map(async (project) => {
            const files: CanvasExportAsset[] = [];
            await Promise.all(
                collectCanvasStorageKeys(project).map(async (storageKey) => {

...
import { canUseCustomChannels, useUserStore, type FeatureAvailability } from "@/stores/use-user-store";
...
function buildNav(features: FeatureAvailability, balance: string, isAdmin: boolean, customChannelsEnabled: boolean): { groups: WorkspaceNavGroup[]; footer: WorkspaceNavItem[] } {
...
toolItem("projects", "/projects"),
    ...
    const settingsSections = SETTINGS_SECTIONS.filter((section) => section.key !== "channels" || customChannelsEnabled);
...
export function WorkspaceSidebarNav({ collapsed, onNavigate, onOpenSearch, onExpand }: { collapsed: boolean; onNavigate: () => void; onOpenSearch: () => void; onExpand: () => void }) {
    ...
    const customChannelsEnabled = canUseCustomChannels(user, features.customChannelsEnabled);
    ...
    const { groups, footer } = useMemo(() => buildNav(features, balance, user?.role === "admin", customChannelsEnabled), [customChannelsEnabled, features, balance, user?.role]);

...
export function useCanvasCreateCommands(context: AddNodeMenuContext, runCommand?: (command: () => void) => void): CanvasCreateCommand[] {
    ...
    const enabledPluginIds = new Set(installations.filter((item) => isPluginEffectivelyEnabled(item.manifest.id, pluginStates[item.manifest.id]?.effectiveEnabled ?? item.enabled)).map((item) => item.manifest.id));

...
export function CanvasConfigNodePanel({ node, isRunning, inputSummary, onConfigChange, onGenerate, onComposerToggle, workspaceMode = "professional" }: CanvasConfigNodePanelProps) {
    ...
    const allowDesktopLocalTools = canUseDesktopLocalTools(user, desktopLocalChannelsEnabled);
    ...
    useEffect(() => {
        ...
        if (workflowProvider === "comfyui" && allowDesktopLocalTools && !node.metadata?.comfyBridgeWorkflowId?.trim()) {
    ...
    }, [allowDesktopLocalTools, globalConfig.comfyBridge.workflows, globalConfig.comfyBridge.workflowId, globalConfig.runningHub.capability, globalConfig.runningHub.selectedKind, globalConfig.runningHub.workflowId, globalConfig.runningHub.workflows, mode, node.id, node.metadata?.comfyBridgeWorkflowId, node.metadata?.runningHubWorkflowId, onConfigChange, workflowCapability, workflowProvider]);
    ...
    const comfyBridgeEntries = globalConfig.comfyBridge.workflows
        .filter((item) => allowDesktopLocalTools && (item.capability === workflowCapability || item.workflowId.trim() === node.metadata?.comfyBridgeWorkflowId?.trim()))
    ...
    const capabilityError = workflowParameterError || (workflowProvider === "runninghub"
        ...
        : workflowProvider === "comfyui"
            ? (!allowDesktopLocalTools ? "ComfyUI Bridge 仅管理员可用" : !workflowProviderPluginEnabled(runtimeStatuses, "comfyui") ? "ComfyUI Bridge 工作流插件未启用" : !globalConfig.comfyBridge.enabled ? "请先在设置中启用 ComfyUI Bridge" : !globalConfig.comfyBridge.bridgeId ? "请选择在线 Bridge" : !node.metadata?.comfyBridgeWorkflowId ? `请选择${capabilityLabel(workflowCapability)}工作流` : !selectedComfyBridgeWorkflow ? "当前画布引用的 ComfyUI 条目已不存在，请重新选择" : selectedComfyBridgeWorkflow.capability !== workflowCapability ? `当前条目用途为${capabilityLabel(selectedComfyBridgeWorkflow.capability)}，请切换画布模式或重新选择条目` : undefined)
            : undefined);
                    ...
                    <div className="flex items-center gap-3" data-canvas-no-zoom onMouseDown={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()}>
                        ...
                        <Segmented
                            ...
                            options={[
                                ...
                                ...(allowDesktopLocalTools && workflowProviderPluginEnabled(runtimeStatuses, "comfyui") ? [{ label: "ComfyUI", value: "comfyui" }] : []),
                            ]}
                            onChange={(value) => {
                                ...
                                if (nextProvider === "runninghub") {
                                    ...
                                    if (!allowDesktopLocalTools) return;

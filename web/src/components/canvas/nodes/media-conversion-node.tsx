...
const MEDIA_CONVERSION_PLUGIN_ID = "media-conversion";
...
export function MediaConversionNodeContent({ node, theme }: MediaConversionNodeContentProps) {
    ...
    const pluginEffectiveEnabled = usePluginStore((store) => store.pluginStates[MEDIA_CONVERSION_PLUGIN_ID]?.effectiveEnabled);
    const pluginInstalledEnabled = usePluginStore((store) => store.installations.find((item) => item.manifest.id === MEDIA_CONVERSION_PLUGIN_ID)?.enabled);
    const pluginRuntimeStatus = usePluginStore((store) => store.runtimeStatuses[MEDIA_CONVERSION_PLUGIN_ID]);
    const enabled = isPluginEffectivelyEnabled(MEDIA_CONVERSION_PLUGIN_ID, pluginEffectiveEnabled ?? (pluginRuntimeStatus === undefined ? pluginInstalledEnabled : pluginRuntimeStatus === "enabled"));
    ...
    useEffect(() => {
        if (!enabled || (!updateNode && !updateMetadata) || !currentFingerprint || state.status !== "completed" || state.sourceFingerprint === currentFingerprint) return;
    ...
    }, [currentFingerprint, enabled, node.id, state, updateMetadata, updateNode]);
    ...
    const selectOperation = (operation: MediaConversionOperation) => {
        if (!enabled || operation === state.operation) return;
        setNotice("");
        updateState({
            operation,
    ...
    const buttonDisabled = !enabled || !input || hasMultipleInputs || status === "processing";
    ...
    const previewStatusText = state.operation === "pose" && status === "completed" && typeof state.detectedPeople === "number"
        ...
        : statusText;
    const showRuntimeRecovery = enabled && (state.operation === "depth" || state.operation === "lineart" || state.operation === "pose") && (localRuntimeConnection === "unreachable" || state.errorCode === "depth_runtime_unavailable" || state.errorCode === "lineart_runtime_unavailable" || state.errorCode === "pose_runtime_unavailable");
    ...
    return (
        ...
        >
            ...
            <div className="flex min-h-4 items-center gap-2 text-[var(--fs-micro)]" style={{ color: status === "unavailable" || status === "skipped" ? "var(--status-warning)" : notice || status === "error" ? "var(--status-error)" : theme.node.muted }}>
                <span className="min-w-0 flex-1 truncate" title={notice || mediaConversionOperationDescription(state.operation)}>{notice || (enabled ? mediaConversionOperationDescription(state.operation) : "本机转换仅管理员可用；已有结果只读")}</span>
                {showRuntimeRecovery ? (
                    <button
                        type="button"

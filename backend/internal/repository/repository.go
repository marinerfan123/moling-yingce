...
// Transaction exposes a repository-scoped transaction to app workflows that
// need several domain records to commit or roll back together.
func (r *Repository) Transaction(fn func(*Repository) error) error {
	if fn == nil {
		return errors.New("nil repository transaction callback")
	}
...
func (r *Repository) CreateUserWithEmailVerification(user *model.User, verificationCodeID string, usedAt time.Time) error {
	return r.CreateUserWithVerifications(user, verificationCodeID, "", usedAt)
}
...
// RecordPhoneVerificationFailure increments the attempt counter atomically.
// A negative result means that the challenge is already expired, used, or locked.
func (r *Repository) RecordPhoneVerificationFailure(id string, now time.Time, maxAttempts int) (int, error) {
	if maxAttempts <= 0 {
		return -1, errors.New("手机验证码失败次数上限无效")
	}
	attempts := -1
	err := r.db.Transaction(func(tx *gorm.DB) error {
		result := tx.Model(&model.PhoneVerificationCode{}).
			Where("id = ? AND used_at IS NULL AND expires_at > ? AND attempts < ?", id, now, maxAttempts).
		...
		if err := tx.Select("attempts").First(&record, "id = ?", id).Error; err != nil {
			return err
		}
...
func (r *Repository) CreateUserWithVerifications(user *model.User, emailVerificationCodeID string, phoneVerificationCodeID string, usedAt time.Time) error {
	return r.db.Transaction(func(tx *gorm.DB) error {
		if emailVerificationCodeID != "" {
			result := tx.Model(&model.EmailVerificationCode{}).Where("id = ? AND used_at IS NULL AND expires_at > ?", emailVerificationCodeID, usedAt).Update("used_at", usedAt)
			if result.Error != nil {
				return result.Error
			}
			if result.RowsAffected != 1 {
		...
		if phoneVerificationCodeID != "" {
			result := tx.Model(&model.PhoneVerificationCode{}).Where("id = ? AND used_at IS NULL AND expires_at > ?", phoneVerificationCodeID, usedAt).Update("used_at", usedAt)
			if result.Error != nil {
				return result.Error
			}
			if result.RowsAffected != 1 {
...
func (r *Repository) Tasks(userID string, limit int, projectID string, activeOnly bool) ([]model.Task, error) {
	...
	query := taskListQuery(r.db, userID, projectID, activeOnly).Limit(limit)
	err := query.Order("created_at desc").Find(&tasks).Error
	return tasks, err
}

// TasksForUser is the unbounded source for service-level policy filtering.
// Applying the local-tool policy before pagination prevents hidden tasks from
// distorting page size and silently pushing visible tasks to later pages.
func (r *Repository) TasksForUser(userID string, projectID string, activeOnly bool) ([]model.Task, error) {
	var tasks []model.Task
	err := taskListQuery(r.db, userID, projectID, activeOnly).Order("created_at desc").Find(&tasks).Error
	return tasks, err
}

func taskListQuery(db *gorm.DB, userID string, projectID string, activeOnly bool) *gorm.DB {
	query := db.Select("id", "user_id", "session_id", "project_id", "type", "status", "stage", "progress", "prompt", "operation", "provider", "model", "input_json", "result_json", "billing_order_id", "provider_request_id", "provider_cancel_status", "provider_cancel_error", "provider_cancel_attempts", "provider_cancel_requested_at", "provider_cancelled_at", "provider_cancel_next_check_at", "attempts", "started_at", "completed_at", "created_at", "updated_at").
...
func (r *Repository) SessionTasks(userID string, sessionID string) ([]model.Task, error) {
	var tasks []model.Task
	err := r.db.Select("id", "user_id", "session_id", "project_id", "type", "status", "stage", "progress", "prompt", "operation", "provider", "model", "input_json", "result_json", "attempts", "started_at", "completed_at", "created_at", "updated_at").
...
func (r *Repository) ProjectCanvasDocuments(userID string, projectID string) ([]model.CanvasProject, error) {
	var canvases []model.CanvasProject
	err := r.db.Select("id", "user_id", "project_id", "title", "payload_json", "created_at", "updated_at").Where("user_id = ? AND project_id = ?", userID, projectID).Order("updated_at desc, id asc").Find(&canvases).Error
	return canvases, err
}

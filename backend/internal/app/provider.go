...
func (s *Service) processCanvasGenerationTask(ctx context.Context, userID string, taskProjectID string, taskType string, fallbackPrompt string, rawInput string) (map[string]interface{}, error) {
	...
	config, err := s.resolveProviderConfigForUser(input.Config, userID)
	if err != nil {
		return nil, err
	}
	input.Config = config
	...
	if isWorkflowProviderInterface(input.Config.InterfaceType) {
		if err := s.RequireWorkflowPluginForUser(userID, input.Config.InterfaceType); err != nil {
			return nil, err
		}
...
func (s *Service) resolveProviderConfigForUser(config providerConfig, userID string) (providerConfig, error) {
	actor, err := s.repo.User(userID)
	if err != nil {
		return providerConfig{}, err
	}
	if config.AllowLocalChannel && !s.DesktopLocalChannelsEnabledForUser(actor) {
		return providerConfig{}, Forbidden("当前账号无权使用本机渠道")
	}
	resolved, err := s.resolveProviderConfig(config)
	if err != nil {
		return providerConfig{}, err
	}
	if resolved.AllowLocalChannel && !s.DesktopLocalChannelsEnabledForUser(actor) {
		return providerConfig{}, Forbidden("当前账号无权使用本机渠道")
	}
	return resolved, nil
}

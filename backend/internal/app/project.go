...
func (s *Service) summarizeProjects(userID string, projects []model.Project) ([]ProjectSummary, error) {
	...
	for _, project := range projects {
		...
		canvases, canvasesErr := s.visibleProjectCanvasDocuments(userID, project.ID)
		if canvasesErr != nil {
			return nil, canvasesErr
		}
		assets, assetsErr := s.ProjectAssets(userID, project.ID)
		if assetsErr != nil {
			return nil, assetsErr
		}
		visibleProject, projectErr := s.sanitizeProjectForUser(userID, project)
		if projectErr != nil {
		...
		result = append(result, ProjectSummary{Project: visibleProject, CanvasCount: len(canvases), AssetCount: int64(len(assets)), UnitCount: len(units), CompletedUnitCount: completed})
	}
	return result, nil
}
...
func (s *Service) ProjectDetail(userID string, id string) (ProjectDetail, error) {
	...
	if tasks, tasksErr := s.repo.SuccessfulWorkflowTasksForProject(userID, project.ID); tasksErr == nil {
		...
		if allowedErr != nil {
			return ProjectDetail{}, allowedErr
		}
		for _, task := range tasks {
			if !allowed && canvas.IsDesktopLocalTask(task) {
				continue
			}
	...
	canvasDocuments, err := s.visibleProjectCanvasDocuments(userID, project.ID)
	if err != nil {
		return ProjectDetail{}, err
	}
	canvases := make([]model.CanvasProject, 0, len(canvasDocuments))
	visibleCanvasIDs := make(map[string]struct{}, len(canvasDocuments))
	for _, document := range canvasDocuments {
		canvases = append(canvases, canvasSummary(document))
		visibleCanvasIDs[document.ID] = struct{}{}
	}
	canvasUnitLinks, err := s.repo.ProjectCanvasUnitLinks(project.ID)
	if err != nil {
		return ProjectDetail{}, err
	}
	filteredCanvasUnitLinks := make([]model.CanvasUnitLink, 0, len(canvasUnitLinks))
	for _, link := range canvasUnitLinks {
		if _, ok := visibleCanvasIDs[link.CanvasID]; ok {
			filteredCanvasUnitLinks = append(filteredCanvasUnitLinks, link)
		}
	}
	...
	visibleProject, err := s.sanitizeProjectForUser(userID, *project)
	if err != nil {
		return ProjectDetail{}, err
	}
	return ProjectDetail{Project: visibleProject, Units: units, Canvases: canvases, CanvasUnitLinks: filteredCanvasUnitLinks, Assets: assets, AssetFolders: assetFolders, Workflows: workflows, Shots: shots, ShotRevisions: shotRevisions, ShotArtifacts: shotArtifacts, ShotReferences: shotReferences, AssetCandidates: candidates, Tasks: tasks}, nil
}
...
func (s *Service) sanitizeProjectForUser(userID string, project model.Project) (model.Project, error) {
	allowed, err := s.desktopLocalToolsAllowedForUserID(userID)
	if err != nil {
		return model.Project{}, err
	}
	if !allowed {
		if isDesktopLocalModelReference(project.DefaultImageModel) {
			project.DefaultImageModel = ""
		}
		if isDesktopLocalModelReference(project.DefaultVideoModel) {
			project.DefaultVideoModel = ""
		}
	}
	return project, nil
}

func isDesktopLocalModelReference(value string) bool {
	value = strings.ToLower(strings.TrimSpace(value))
	return strings.HasPrefix(value, "local:dreamina-cli:") || strings.HasPrefix(value, "local:comfyui:")
}

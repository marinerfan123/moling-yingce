...
func TestCustomChannelsAreRestrictedToActiveAdministrators(t *testing.T) {
	svc, _ := newFeatureAvailabilityTestService(t)
	...
	if _, err := svc.UpdateFeatureAvailability(admin, FeatureAvailability{
		...
		CustomChannelsEnabled: true, PluginCenterEnabled: true, SystemPluginsVisibleToUsers: true,
	}); err != nil {
		t.Fatal(err)
	}

	for name, actor := range map[string]*model.User{
		"ordinary user":  {ID: "user-1", Role: model.UserRoleUser, Status: model.UserStatusActive},
		"disabled admin": {ID: "admin-2", Role: model.UserRoleAdmin, Status: model.UserStatusDisabled},
	} {
		t.Run(name, func(t *testing.T) {
			if svc.CustomChannelsEnabledForUser(actor) {
				t.Fatalf("CustomChannelsEnabledForUser(%s) = true", name)
			}
			if err := svc.RequireCustomChannelsForUser(actor); err == nil {
				t.Fatalf("RequireCustomChannelsForUser(%s) error = nil", name)
			}
			features, err := svc.FeatureAvailabilityForUser(actor)
			if err != nil {
				t.Fatal(err)
			}
			if features.CustomChannelsEnabled {
				t.Fatalf("FeatureAvailabilityForUser(%s) exposed custom channels", name)
			}
		})
	}

	if !svc.CustomChannelsEnabledForUser(admin) {
		t.Fatal("active admin should retain custom channel access")
	}
	if err := svc.RequireCustomChannelsForUser(admin); err != nil {
		t.Fatalf("RequireCustomChannelsForUser(admin) error = %v", err)
	}
}

func TestCustomChannelTaskInputRequiresActiveAdministrator(t *testing.T) {
	svc, db := newFeatureAvailabilityTestService(t)
	admin := &model.User{ID: "admin-1", Username: "feature-admin", Role: model.UserRoleAdmin, Status: model.UserStatusActive}
	user := &model.User{ID: "user-1", Username: "feature-user", Role: model.UserRoleUser, Status: model.UserStatusActive}
	...
	input, err := normalizeTaskInput(map[string]any{"config": providerConfig{BaseURL: "https://example.com", APIKey: "private-key", Model: "text-model"}})
	if err != nil {
		t.Fatal(err)
	}
	if err := svc.requireCustomChannelsForUserID(user.ID, input); err == nil {
		t.Fatal("ordinary user custom channel task must be rejected")
	}
	if err := svc.requireCustomChannelsForUserID(admin.ID, input); err != nil {
		t.Fatalf("active admin custom channel task error = %v", err)
	}
}

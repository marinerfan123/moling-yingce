...
// AdminChannelExport is a portable channel configuration snapshot. Credentials
// and custom header values are deliberately represented only by presence flags.
type AdminChannelExport struct {
	Version    int                         `json:"version"`
	ExportedAt time.Time                   `json:"exportedAt"`
	Channels   []AdminChannelExportChannel `json:"channels"`
}

type AdminChannelExportChannel struct {
	ID                string                    `json:"id"`
	...
	Models            []AdminChannelExportModel `json:"models"`
	CreatedAt         time.Time                 `json:"createdAt"`
	UpdatedAt         time.Time                 `json:"updatedAt"`
}

type AdminChannelExportModel struct {
	ID                           string                        `json:"id"`
	...
	PriceTiers                   []AdminChannelExportPriceTier `json:"priceTiers"`
}

type AdminChannelExportPriceTier struct {
...
type AdminChannelImportRequest struct {
	Version  int                         `json:"version"`
	Channels []AdminChannelExportChannel `json:"channels"`
}

type AdminChannelImportResult struct {
	ImportedChannels int `json:"importedChannels"`
	ImportedModels   int `json:"importedModels"`
}

const (
	maxAdminChannelImportChannels = 100
	maxAdminChannelImportModels   = 2000
	maxAdminChannelImportTiers    = 100
)
...
func (s *Service) ExportAdminSystemChannels(actor *model.User) (*AdminChannelExport, error) {
	if err := s.RequireAdmin(actor); err != nil {
		return nil, err
	}
	...
	result := &AdminChannelExport{
		Version:    1,
		ExportedAt: time.Now().UTC(),
		Channels:   make([]AdminChannelExportChannel, 0, len(channels)),
	}
	for _, channel := range channels {
		items, itemErr := s.repo.ChannelModels(channel.ID, true)
		if itemErr != nil {
		...
		exported := AdminChannelExportChannel{
			ID:                channel.ID,
			Name:              channel.Name,
			PublicAlias:       channel.PublicAlias,
			SortOrder:         channel.SortOrder,
			BaseURL:           channel.BaseURL,
			APIFormat:         channel.APIFormat,
			AllowLocalChannel: channel.AllowLocalChannel,
			...
			Models:            make([]AdminChannelExportModel, 0, len(items)),
			CreatedAt:         channel.CreatedAt,
			UpdatedAt:         channel.UpdatedAt,
		}
		for _, item := range items {
			exported.Models = append(exported.Models, exportAdminChannelModel(item))
		}
		result.Channels = append(result.Channels, exported)
	}
	return result, nil
}

func exportAdminChannelModel(item model.ChannelModel) AdminChannelExportModel {
	...
	if decodeErr == nil && capabilityConfig != nil {
		...
		capabilityConfig = nil
	}

	tiers := make([]AdminChannelExportPriceTier, 0, len(item.PriceTiers))
	for _, tier := range item.PriceTiers {
		tiers = append(tiers, AdminChannelExportPriceTier{
			...
			PriceVersion:                 tier.PriceVersion,
		})
	}
	return AdminChannelExportModel{
		ID:                           item.ID,
...
func (s *Service) ImportAdminSystemChannels(actor *model.User, req AdminChannelImportRequest) (*AdminChannelImportResult, error) {
	if err := s.RequireAdmin(actor); err != nil {
		return nil, err
	}
	if req.Version != 0 && req.Version != 1 {
		return nil, BadAuthRequest("不支持的渠道配置文件版本")
	}
	if len(req.Channels) == 0 {
		return nil, BadAuthRequest("导入文件中没有渠道")
	}
	if len(req.Channels) > maxAdminChannelImportChannels {
		return nil, BadAuthRequest(fmt.Sprintf("一次最多导入 %d 个渠道", maxAdminChannelImportChannels))
	}

	existing, err := s.repo.SystemChannels(true)
	if err != nil {
		return nil, err
	}
	seenNames := make(map[string]struct{}, len(existing)+len(req.Channels))
	seenBaseURLs := make(map[string]struct{}, len(existing)+len(req.Channels))
	for _, channel := range existing {
		if key := normalizeAdminChannelImportName(channel.Name); key != "" {
			seenNames[key] = struct{}{}
		}
		if key := normalizeAdminChannelImportBaseURL(channel.BaseURL); key != "" {
			seenBaseURLs[key] = struct{}{}
		}
	}

	totalModels := 0
	for index, input := range req.Channels {
		name := strings.TrimSpace(input.Name)
		if name == "" {
			return nil, BadAuthRequest(fmt.Sprintf("第 %d 个渠道缺少名称", index+1))
		}
		if utf8.RuneCountInString(name) > 80 {
			return nil, BadAuthRequest(fmt.Sprintf("第 %d 个渠道名称不能超过 80 个字符", index+1))
		}
		baseURL := strings.TrimSpace(input.BaseURL)
		if baseURL == "" {
			return nil, BadAuthRequest(fmt.Sprintf("渠道 %s 缺少 Base URL", name))
		}
		if _, err := s.validateChannelOutboundURL(baseURL, input.AllowLocalChannel, false); err != nil {
			return nil, err
		}
		if nameKey := normalizeAdminChannelImportName(name); nameKey != "" {
			if _, exists := seenNames[nameKey]; exists {
				return nil, BadAuthRequest("导入渠道名称已存在或重复：" + name)
			}
			seenNames[nameKey] = struct{}{}
		}
		if baseKey := normalizeAdminChannelImportBaseURL(baseURL); baseKey != "" {
			if _, exists := seenBaseURLs[baseKey]; exists {
				return nil, BadAuthRequest("导入渠道 Base URL 已存在或重复：" + baseURL)
			}
			seenBaseURLs[baseKey] = struct{}{}
		}
		if input.PublicAlias != "" && utf8.RuneCountInString(strings.TrimSpace(input.PublicAlias)) > 80 {
			return nil, BadAuthRequest("渠道前台显示别名不能超过 80 个字符：" + name)
		}
		if input.ConcurrencyLimit < 0 || input.ConcurrencyLimit > maxChannelConcurrencyLimit {
			return nil, BadAuthRequest("渠道最大并发数必须是 0-999 的整数：" + name)
		}
		if len(input.HeaderNames) > 32 {
			return nil, BadAuthRequest("自定义请求头最多支持 32 项：" + name)
		}
		if len(input.Models) > maxAdminChannelImportModels {
			return nil, BadAuthRequest(fmt.Sprintf("渠道 %s 的模型数量超过 %d 个", name, maxAdminChannelImportModels))
		}
		totalModels += len(input.Models)
		if totalModels > maxAdminChannelImportModels {
			return nil, BadAuthRequest(fmt.Sprintf("一次最多导入 %d 个模型", maxAdminChannelImportModels))
		}
		if apiFormat := strings.TrimSpace(input.APIFormat); utf8.RuneCountInString(apiFormat) > 24 {
			return nil, BadAuthRequest("渠道 API 格式不能超过 24 个字符：" + name)
		}
		for modelIndex, exported := range input.Models {
			modelKey := strings.TrimPrefix(strings.TrimSpace(exported.ModelKey), "models/")
			if modelKey == "" {
				return nil, BadAuthRequest(fmt.Sprintf("渠道 %s 的第 %d 个模型缺少模型标识", name, modelIndex+1))
			}
			if utf8.RuneCountInString(modelKey) > 120 || utf8.RuneCountInString(strings.TrimSpace(exported.ProviderModelKey)) > 120 {
				return nil, BadAuthRequest("渠道模型标识不能超过 120 个字符：" + modelKey)
			}
			if len(exported.PriceTiers) > maxAdminChannelImportTiers {
				return nil, BadAuthRequest("渠道模型价格档数量过多：" + modelKey)
			}
		}
	}

	result := &AdminChannelImportResult{}
	err = s.repo.Transaction(func(txRepo *repository.Repository) error {
		transactionService := *s
		transactionService.repo = txRepo
		for _, input := range req.Channels {
			name := strings.TrimSpace(input.Name)
			publicAlias := strings.TrimSpace(input.PublicAlias)
			baseURL := strings.TrimSpace(input.BaseURL)
			allowLocal := input.AllowLocalChannel
			enabled := false
			useGlobalConcurrency := input.ConcurrencyLimit == 0
			channelID, idErr := txRepo.NextPrefixedID("CHANNEL")
			if idErr != nil {
				return idErr
			}
			channel, channelErr := transactionService.channelFromRequest(ChannelRequest{
				Name:                 name,
				PublicAlias:          &publicAlias,
				SortOrder:            &input.SortOrder,
				BaseURL:              baseURL,
				AllowLocalChannel:    &allowLocal,
				UseGlobalConcurrency: &useGlobalConcurrency,
				ConcurrencyLimit:     &input.ConcurrencyLimit,
				Headers:              adminChannelImportHeaders(input.HeaderNames),
				Enabled:              &enabled,
			}, model.ModelChannel{ID: channelID, UserID: actor.ID, Scope: model.ChannelScopeSystem, Enabled: false})
			if channelErr != nil {
				return channelErr
			}
			channel.APIFormat = strings.TrimSpace(input.APIFormat)
			...
			for _, exported := range input.Models {
				item, modelErr := transactionService.saveAdminChannelModel(actor, channel.ID, "", adminChannelImportModelRequest(exported), false, false)
				if modelErr != nil {
					return modelErr
				}
				item.SortOrder = exported.SortOrder
				if err := txRepo.SaveChannelModel(item); err != nil {
					return err
				}
				result.ImportedModels++
			}
			if err := txRepo.SyncChannelModelNames(channel.ID, time.Now()); err != nil {
				return err
			}
...
func adminChannelImportHeaders(names []string) []OutboundHeader {
	headers := make([]OutboundHeader, 0, len(names))
	for _, name := range names {
		headers = append(headers, OutboundHeader{Name: name, Value: ""})
	}
	return headers
}

func adminChannelImportModelRequest(exported AdminChannelExportModel) ChannelModelRequest {
	modelEnabled := exported.Enabled
	tiers := make([]ChannelModelPriceTierRequest, 0, len(exported.PriceTiers))
	for _, tier := range exported.PriceTiers {
	...
	return ChannelModelRequest{
		ModelKey:                     exported.ModelKey,
		ProviderModelKey:             exported.ProviderModelKey,
		DisplayName:                  exported.DisplayName,
		Icon:                         exported.Icon,
		Capability:                   exported.Capability,
		Protocol:                     string(exported.Protocol),
		BillingMode:                  exported.BillingMode,
		UnitPriceMicrocredits:        exported.UnitPriceMicrocredits,
		InputTokenPriceMicrocredits:  exported.InputTokenPriceMicrocredits,
		OutputTokenPriceMicrocredits: exported.OutputTokenPriceMicrocredits,
		CachedTokenPriceMicrocredits: exported.CachedTokenPriceMicrocredits,
		PriceConfigured:              exported.PriceConfigured,
		Enabled:                      &modelEnabled,
		CapabilityConfig:             exported.CapabilityConfig,
		PriceTiers:                   tiers,
	}
}

func normalizeAdminChannelImportName(value string) string {
	return strings.ToLower(strings.TrimSpace(value))
}

func normalizeAdminChannelImportBaseURL(value string) string {

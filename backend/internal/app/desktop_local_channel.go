...
// DesktopLocalChannelsEnabledForUser keeps the deployment capability separate
// from the account authorization. A disabled account must fail closed too.
func (s *Service) DesktopLocalChannelsEnabledForUser(user *model.User) bool {
	return s.DesktopLocalChannelsEnabled() && user != nil && user.Role == model.UserRoleAdmin && user.Status == model.UserStatusActive
}

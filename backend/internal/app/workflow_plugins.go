...
func (s *Service) WorkflowPluginStatusesForUser(userID string) (map[string]string, error) {
	...
	return s.WorkflowPluginStatusesForActor(actor)
}

func (s *Service) WorkflowPluginStatusesForActor(actor *model.User) (map[string]string, error) {
	statuses := make(map[string]string, 2)
	for _, pluginID := range []string{WorkflowPluginRunningHub, WorkflowPluginComfyUI} {
		// ComfyUI Bridge is a desktop-local capability. Keep its discovery state
		// fail-closed for ordinary users so the canvas cannot offer a provider
		// that the server will reject at execution time.
...
func (s *Service) RequireWorkflowPluginForActor(actor *model.User, interfaceType string) error {
	pluginID, ok := workflowPluginIDForInterface(normalizeWorkflowInterfaceType(interfaceType))
	if !ok {
		return Forbidden("未知工作流插件")
	}
	if err := s.RequirePluginForActor(actor, pluginID); err != nil {
		if pluginID == WorkflowPluginRunningHub {
			return Forbidden("RunningHub 工作流插件未启用")
		}
		return Forbidden("ComfyUI Bridge 工作流插件未启用")

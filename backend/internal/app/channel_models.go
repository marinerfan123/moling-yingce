...
func (s *Service) SaveAdminChannelModel(actor *model.User, channelID string, id string, req ChannelModelRequest) (*model.ChannelModel, error) {
	return s.saveAdminChannelModel(actor, channelID, id, req, true, true)
}

func (s *Service) saveAdminChannelModel(actor *model.User, channelID string, id string, req ChannelModelRequest, requireChannelCredentials bool, invalidateRouteCatalog bool) (*model.ChannelModel, error) {
	if err := s.RequireAdmin(actor); err != nil {
		return nil, err
	}
	...
	modelKey, providerModelKey, capability, protocol, err := s.normalizeChannelModelContractWithCredentials(channel, req, requireChannelCredentials)
	if err != nil {
		return nil, err
	}
...
func normalizeChannelModelContract(channel *model.ModelChannel, req ChannelModelRequest) (string, string, string, model.ChannelInterfaceType, error) {
	return normalizeChannelModelContractWithRegistry(protocol.Builtins(), channel, req, true)
}
...
func (s *Service) normalizeChannelModelContract(channel *model.ModelChannel, req ChannelModelRequest) (string, string, string, model.ChannelInterfaceType, error) {
	return s.normalizeChannelModelContractWithCredentials(channel, req, true)
}

func (s *Service) normalizeChannelModelContractWithCredentials(channel *model.ModelChannel, req ChannelModelRequest, requireChannelCredentials bool) (string, string, string, model.ChannelInterfaceType, error) {
	return normalizeChannelModelContractWithRegistry(s.protocolRegistry(), channel, req, requireChannelCredentials)
}

func normalizeChannelModelContractWithRegistry(registry *protocol.Registry, channel *model.ModelChannel, req ChannelModelRequest, requireChannelCredentials bool) (string, string, string, model.ChannelInterfaceType, error) {
	...
	if requireChannelCredentials && (protocol == model.ChannelInterfaceVolcengineJiMengImage || protocol == model.ChannelInterfaceVolcengineJiMengVideo) && (strings.TrimSpace(channel.APIKey) == "" || strings.TrimSpace(channel.SecretKey) == "") {

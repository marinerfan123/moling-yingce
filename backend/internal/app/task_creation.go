...
func (s *Service) CreateTask(userID string, req CreateTaskRequest) (*model.Task, error) {
	...
	if err := s.requireCustomChannelsForUserID(userID, normalizedInput); err != nil {
		return nil, err
	}
	...
	if s.taskInputUsesLocalChannel(normalizedInput) {
		actor, actorErr := s.repo.User(userID)
		if actorErr != nil {
			return nil, actorErr
		}
		if !s.DesktopLocalChannelsEnabledForUser(actor) {
			return nil, Forbidden("当前账号无权使用本机渠道")
		}
	}
...
func taskInputRequestsLocalChannel(input map[string]any) bool {
	config, ok := input["config"].(map[string]any)
	if !ok {
		return false
	}
	if value, ok := config["allowLocalChannel"].(bool); ok && value {
		return true
	}
	modelName := strings.TrimSpace(stringValue(config["model"]))
	return strings.HasPrefix(modelName, "local:dreamina-cli:")
}

func (s *Service) taskInputUsesLocalChannel(input map[string]any) bool {
	if taskInputRequestsLocalChannel(input) {
		return true
	}
	config, ok := input["config"].(map[string]any)
	if !ok {
	...
	return err == nil && channel.AllowLocalChannel
}
...
func (s *Service) requireCustomChannelsForUserID(userID string, input map[string]any) error {
	if !taskInputUsesCustomChannel(input) {
		return nil
	}
	...
	actor, err := s.repo.User(strings.TrimSpace(userID))
	if err != nil {
		return err
	}
	return s.RequireCustomChannelsForUser(actor)
}
...
func (s *Service) resolveSystemChannelModelSelection(input map[string]any, taskType string, operation string) (map[string]any, error) {
	...
	nextConfig["allowLocalChannel"] = channel.AllowLocalChannel
	return input, nil
}

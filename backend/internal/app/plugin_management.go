...
func isDesktopLocalPluginID(pluginID string) bool {
	switch strings.TrimSpace(pluginID) {
	case PluginEagleAssetConnector, PluginPortraitClearance, PluginMediaConversion, WorkflowPluginComfyUI:
		return true
	default:
		return false
	}
...
func isActiveAdmin(actor *model.User) bool {
	return actor != nil && strings.TrimSpace(actor.ID) != "" && actor.Role == model.UserRoleAdmin && actor.Status == model.UserStatusActive
}
...
func (s *Service) PluginStatesForUser(actor *model.User) (map[string]PluginStateView, error) {
	...
	for _, pluginID := range knownPluginIDs(items) {
		if isDesktopLocalPluginID(pluginID) && !isActiveAdmin(actor) {
			continue
		}
...
func (s *Service) pluginStateForUser(actor *model.User, pluginID string, items []PluginView) (PluginStateView, error) {
	...
	if isDesktopLocalPluginID(pluginID) && !s.DesktopLocalChannelsEnabledForUser(actor) {
...
func (s *Service) RequirePluginForActor(actor *model.User, pluginID string) error {
	if isDesktopLocalPluginID(pluginID) {
		if !isActiveAdmin(actor) {
			return Forbidden("本机插件仅管理员可用")
		}
		if !s.DesktopLocalChannelsEnabledForUser(actor) {
			return Forbidden("本机工具未由当前部署启用")
		}
	}
...
// RequirePluginForUser keeps ID-only workflow callers compatible while still
// resolving the persisted role before checking desktop-local plugins.
func (s *Service) RequirePluginForUser(userID string, pluginID string) error {
	actor := &model.User{ID: strings.TrimSpace(userID), Role: model.UserRoleUser, Status: model.UserStatusActive}
	if s.repo != nil {
		if stored, err := s.repo.User(actor.ID); err == nil {
			actor = stored
		}
	}
	return s.RequirePluginForActor(actor, pluginID)
}

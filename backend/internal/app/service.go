...
func (s *Service) SessionDetail(userID string, id string) (*SessionDetail, error) {
	...
	for _, task := range tasks {
		...
		visibleTaskIDs[task.ID] = struct{}{}
	}
	if !allowed {
		if strings.TrimSpace(session.CanvasSnapshotJSON) != "" {
			onlyLocal, filterErr := canvas.IsDesktopLocalOnlyCanvasPayload(session.CanvasSnapshotJSON)
			if filterErr != nil {
				return nil, filterErr
			}
			filtered, filterErr := canvas.FilterCanvasPayload(session.CanvasSnapshotJSON, false)
			if filterErr != nil {
				return nil, filterErr
			}
			if onlyLocal {
				session.CanvasSnapshotJSON = "{}"
			} else {
				session.CanvasSnapshotJSON = filtered
			}
		}
		session.CanvasOpsJSON = ""
	}
	taskSummaries := taskSummariesForOutput(visibleTasks)
	results, err := s.repo.SessionResults(userID, id)
	if err != nil {
		return nil, err
	}
	if !allowed {
		visibleResults := make([]model.Result, 0, len(results))
		for _, result := range results {
			if _, ok := visibleTaskIDs[result.TaskID]; ok || strings.TrimSpace(result.TaskID) == "" {
...
func (s *Service) TasksWithOptions(userID string, options TaskListOptions) ([]TaskSummary, error) {
	tasks, err := s.repo.TasksForUser(userID, options.ProjectID, options.ActiveOnly)
	if err != nil {
		return nil, err
	}
	allowed, err := s.desktopLocalToolsAllowedForUserID(userID)

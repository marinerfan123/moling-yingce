...
func (s *Service) FeatureAvailabilityForUser(actor *model.User) (*PublicFeatureAvailability, error) {
	features, err := s.FeatureAvailability()
	if err != nil {
		return nil, err
	}
	features.DesktopLocalChannelsEnabled = s.DesktopLocalChannelsEnabledForUser(actor)
	features.CustomChannelsEnabled = features.CustomChannelsEnabled && isActiveAdmin(actor)
	return features, nil
}

// CustomChannelsEnabledForUser limits personal model providers to active
// administrators even when the platform-wide capability is enabled.
func (s *Service) CustomChannelsEnabledForUser(actor *model.User) bool {
	if !isActiveAdmin(actor) {
		return false
	}
	enabled, err := s.FeatureEnabled(FeatureCustomChannels)
	return err == nil && enabled
}

// RequireCustomChannelsForUser is the authorization boundary for personal
// providers and their relay/model-discovery endpoints.
func (s *Service) RequireCustomChannelsForUser(actor *model.User) error {
	if actor == nil || actor.ID == "" {
		return Unauthorized("请先登录")
	}
	if !isActiveAdmin(actor) {
		return Forbidden("自定义渠道仅管理员可用")
	}

...
func (s *Service) desktopLocalToolsAllowedForUserID(userID string) (bool, error) {
	return (canvasHost{svc: s}).DesktopLocalToolsAllowed(userID)
}

func (h canvasHost) DesktopLocalToolsAllowed(userID string) (bool, error) {
	if h.svc == nil || !h.svc.DesktopLocalChannelsEnabled() {
		return false, nil
	}
	user, err := h.svc.repo.User(userID)
	...
	return h.svc.DesktopLocalChannelsEnabledForUser(user), nil
}

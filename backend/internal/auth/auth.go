...
type PublicAuthSettings struct {
	...
	PhoneVerificationEnabled   bool `json:"phoneVerificationEnabled"`
	PhoneVerificationAvailable bool `json:"phoneVerificationAvailable"`
	PhoneCodeRequired          bool `json:"phoneCodeRequired"`
}
...
func (s *Service) PublicAuthSettings() (*PublicAuthSettings, error) {
	...
	_, emailSetting, err := s.readEmailSetting()
	if err != nil {
		return nil, err
	}
	emailEnabled := emailSettingConfigured(emailSetting)
	_, smsSetting, err := s.readSMSSetting()
	if err != nil {
		return nil, err
	}
	return &PublicAuthSettings{
		FirstUser: false, RegistrationEnabled: registrationEnabled, LinuxDOEnabled: s.LinuxDOEnabled(),
		WeChatEnabled: s.WeChatEnabled(), DouyinEnabled: s.DouyinEnabled(), EmailEnabled: emailEnabled, EmailCodeRequired: emailSetting.Enabled,
		PhoneVerificationEnabled: smsSetting.Enabled, PhoneVerificationAvailable: smsSetting.Enabled && smsSettingConfigured(smsSetting), PhoneCodeRequired: smsSetting.Enabled,
	}, nil
}
...
func (s *Service) Register(req RegisterRequest) (*AuthSessionResult, error) {
	...
	if req.Phone != "" && phone == "" {
		return nil, kernel.BadAuthRequest("请输入有效的中国大陆手机号")
	}
	...
	var verifiedPhoneCode *model.PhoneVerificationCode
	if count > 0 {
		registrationEnabled, err := s.RegistrationEnabled()
		if err != nil {
			return nil, err
		...
		emailRequired, err := s.EmailVerificationRequired()
		if err != nil {
			return nil, err
		}
		if emailRequired {
		...
		phoneRequired, err := s.SMSVerificationRequired()
		if err != nil {
			return nil, err
		}
		if phoneRequired {
			...
			verifiedPhoneCode, err = s.VerifyRegistrationPhoneCode(phone, req.PhoneCode)
			if err != nil {
				return nil, err
			}
		} else if phone != "" {
			return nil, kernel.BadAuthRequest("手机验证码验证未启用，不能绑定手机号")
		}
	}
	...
	if verifiedPhoneCode != nil {
		user.Phone = phone
	}
	if verifiedCode != nil || verifiedPhoneCode != nil {
		if err := s.repo.CreateUserWithVerifications(&user, verificationCodeID(verifiedCode), phoneVerificationCodeID(verifiedPhoneCode), time.Now()); err != nil {
			if phone != "" && isPhoneUniqueConstraintError(err) {
				return nil, kernel.BadAuthRequest("手机号已被注册")
			}
			return nil, err
		}
...
func isPhoneUniqueConstraintError(err error) bool {
	...
	return strings.Contains(message, "idx_users_phone_nonempty") ||
		strings.Contains(message, "users.phone") ||
		(strings.Contains(message, "unique") && strings.Contains(message, "phone"))
}

func verificationCodeID(record *model.EmailVerificationCode) string {
	if record == nil {
		return ""
	}
	return record.ID
...
func phoneVerificationCodeID(record *model.PhoneVerificationCode) string {
	if record == nil {
		return ""
	}
	return record.ID
...
func (s *Service) PublicAuthUser(user *model.User) (AuthUser, error) {
	...
	for _, provider := range []string{"linuxdo", "wechat", "douyin"} {

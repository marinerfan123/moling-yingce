...
func (s *Service) EmailEnabled() (bool, error) {
	...
	return emailSettingConfigured(value), nil
}

// EmailVerificationRequired reports the administrator's registration switch.
// It deliberately does not include SMTP completeness; callers use EmailEnabled
// separately to distinguish an enabled but unusable mail configuration.
func (s *Service) EmailVerificationRequired() (bool, error) {
	_, value, err := s.readEmailSetting()
	if err != nil {
		return false, err
	}
...
func emailSettingConfigured(value EmailSettingValue) bool {
	return value.Enabled && value.Host != "" && value.Port > 0 && value.FromEmail != ""
}

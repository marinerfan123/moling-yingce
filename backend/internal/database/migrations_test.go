...
func TestMigrateSchemaRecordsAndValidatesVersion(t *testing.T) {
	...
	if !db.Migrator().HasColumn(&model.PhoneVerificationCode{}, "Attempts") {
		t.Fatal("schema migration v12 did not add phone verification attempts")
	}
	if !db.Migrator().HasIndex(&model.User{}, "idx_users_phone_nonempty") {
		t.Fatal("schema migration v11 did not create the non-empty phone unique index")
	}
	if err := db.Create(&model.User{ID: "phone-user-1", Username: "phone-user-1", Phone: "13800138000"}).Error; err != nil {
		t.Fatal(err)
	}
	if err := db.Create(&model.User{ID: "phone-user-2", Username: "phone-user-2", Phone: "13800138000"}).Error; err == nil {
		t.Fatal("duplicate non-empty phone number should be rejected")
	}
	if err := MigrateSchema(db); err != nil {

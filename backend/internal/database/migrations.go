...
const phoneVerificationChecksum = "sha256:phone-verification-v11-20260913"
const phoneVerificationAttemptsChecksum = "sha256:phone-verification-attempts-v12-20260913"
...
var schemaMigrations = []migration{
	...
	{version: 11, name: "phone_verification", checksum: phoneVerificationChecksum, apply: migrateSchemaV11},
	{version: 12, name: "phone_verification_attempts", checksum: phoneVerificationAttemptsChecksum, apply: migrateSchemaV12},
}
...
func migrateSchemaV11(tx *gorm.DB) error {
	if err := tx.AutoMigrate(&model.User{}, &model.PhoneVerificationCode{}); err != nil {
		return fmt.Errorf("创建手机验证码注册结构：%w", err)
	}
	if err := tx.Exec("CREATE UNIQUE INDEX IF NOT EXISTS idx_users_phone_nonempty ON users(phone) WHERE phone <> ''").Error; err != nil {
		return fmt.Errorf("创建手机号唯一索引：%w", err)
	}
	return nil
}

func migrateSchemaV12(tx *gorm.DB) error {
	if err := tx.AutoMigrate(&model.PhoneVerificationCode{}); err != nil {
		return fmt.Errorf("增加手机验证码失败次数：%w", err)
	}
	return nil
}

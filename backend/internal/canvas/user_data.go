...
func (s *Service) desktopLocalToolsAllowed(userID string) (bool, error) {
	return s.host.DesktopLocalToolsAllowed(strings.TrimSpace(userID))
}

func (s *Service) filteredCanvasPayload(userID string, raw string) (string, bool, error) {
	...
	onlyLocal, err := IsDesktopLocalOnlyCanvasPayload(raw)
	if err != nil {
		return "", false, err
	}
	if onlyLocal {
	...
	filtered, err := FilterCanvasPayload(raw, false)
	return filtered, false, err
}

func (s *Service) rejectDesktopLocalCanvasPayload(userID string, raw string) error {
	allowed, err := s.desktopLocalToolsAllowed(userID)
	if err != nil {
		return err
	}
	...
	containsLocal, err := CanvasPayloadContainsDesktopLocalNode(string(raw))
	if err != nil {
		return kernel.BadAuthRequest("画布数据格式错误")
	}
	if containsLocal {
		return kernel.Forbidden("普通用户不能保存本机工具内容")
	}
	return nil
}
...
func (s *Service) UpsertUserAsset(userID string, raw json.RawMessage) (UserDataSummary, error) {
	...
	if !allowed && IsDesktopLocalAsset(asset) {
		return UserDataSummary{}, kernel.Forbidden("普通用户不能保存本机工具素材")
	}
...
func (s *Service) ReplaceUserAssets(userID string, req AssetsSyncRequest) ([]json.RawMessage, error) {
	...
	if !allowed {
		for _, asset := range assets {
			if IsDesktopLocalAsset(asset) {
				return nil, kernel.Forbidden("普通用户不能保存本机工具素材")
			}
		}
	}
	err = s.host.WithStorageLock(func() error {
...
func (s *Service) UserCanvasProjectSummaries(userID string) ([]UserDataSummary, error) {
	...
	for _, project := range projects {
		_, onlyLocal, filterErr := s.filteredCanvasPayloadWithPermission(allowed, project.PayloadJSON)
		if filterErr != nil {
			return nil, filterErr
		}
		if onlyLocal {
...
func (s *Service) UserCanvasProject(userID string, id string) (json.RawMessage, error) {
	...
	payload, onlyLocal, err := s.filteredCanvasPayload(userID, project.PayloadJSON)
	if err != nil {
		return nil, err
	}
	if onlyLocal {
...
func (s *Service) UpsertUserCanvasProject(userID string, raw json.RawMessage) (UserDataSummary, error) {
	...
	if err := s.rejectDesktopLocalCanvasPayload(userID, string(raw)); err != nil {
		return UserDataSummary{}, err
	}
	err = s.host.WithStorageLock(func() error {
...
func (s *Service) ReplaceUserCanvasProjects(userID string, req CanvasProjectsSyncRequest) ([]json.RawMessage, error) {
	...
	if !allowed {
		for _, raw := range req.Projects {
			containsLocal, parseErr := CanvasPayloadContainsDesktopLocalNode(string(raw))
			if parseErr != nil {
				return nil, kernel.BadAuthRequest("画布数据格式错误")
			}
			if containsLocal {
				return nil, kernel.Forbidden("普通用户不能保存本机工具内容")
			}
		}
	}
	err = s.host.WithStorageLock(func() error {
...
func (s *Service) filteredCanvasPayloadWithPermission(allowed bool, raw string) (string, bool, error) {

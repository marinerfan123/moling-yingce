...
func publicCanvasProject(project *model.CanvasProject, token string) (map[string]any, map[string]bool, error) {
	...
	for _, rawNode := range rawNodes {
		...
		if isDesktopLocalCanvasNodeValue(node) {
			continue
		}
		if node, ok := publicCanvasNode(rawNode, token, allowedResources); ok {
			nodes = append(nodes, node)
			visibleNodeIDs[kernel.StringValue(node["id"])] = true
		}
	}
	for _, rawNode := range nodes {
		node, _ := rawNode.(map[string]any)
		if parentID := kernel.StringValue(node["parentId"]); parentID != "" && !visibleNodeIDs[parentID] {
			delete(node, "parentId")
		}
	}
	result["nodes"] = nodes
	result["connections"] = publicCanvasConnections(source["connections"], visibleNodeIDs)
	return result, allowedResources, nil
}

func publicCanvasConnections(value any, visibleNodeIDs map[string]bool) []any {
	...
	for _, raw := range rawConnections {
		...
		if !visibleNodeIDs[kernel.StringValue(connection["fromNodeId"])] || !visibleNodeIDs[kernel.StringValue(connection["toNodeId"])] {
			continue
		}

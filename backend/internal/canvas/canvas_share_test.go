...
func TestPublicCanvasProjectDropsDesktopLocalNodesAndConnections(t *testing.T) {
	payload := map[string]any{
		"id": "p",
		"nodes": []any{
			map[string]any{"id": "online", "type": "image", "title": "online", "position": map[string]any{"x": 0, "y": 0}, "width": 10, "height": 10, "metadata": map[string]any{"prompt": "keep"}},
			map[string]any{"id": "local", "type": "image", "title": "local", "position": map[string]any{"x": 20, "y": 0}, "width": 10, "height": 10, "metadata": map[string]any{"taskProvider": "dreamina-cli", "content": "resource:resource_local"}},
		},
		"connections": []any{
			map[string]any{"id": "online-edge", "fromNodeId": "online", "toNodeId": "online"},
			map[string]any{"id": "local-edge", "fromNodeId": "online", "toNodeId": "local"},
		},
	}
	raw, _ := json.Marshal(payload)
	public, resources, err := publicCanvasProject(&model.CanvasProject{ID: "p", PayloadJSON: string(raw)}, "share-token")
	if err != nil {
		t.Fatal(err)
	}
	encoded, _ := json.Marshal(public)
	if strings.Contains(string(encoded), "resource_local") || strings.Contains(string(encoded), "dreamina-cli") || len(resources) != 0 {
		t.Fatalf("public payload leaked local node: %s", encoded)
	}
	connections, _ := public["connections"].([]any)
	if len(connections) != 1 {
		t.Fatalf("expected only visible connection, got %v", connections)
	}
}

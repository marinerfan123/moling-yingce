	...
	for _, item := range manifestArray(value) {
		...
		case map[string]any:
			...
			if reference.DataURL == "" {
				if dataURL := firstString(typed, "data_url"); dataURL != "" {
					reference.DataURL = dataURL
				} else if encoded := firstString(typed, "b64_json"); encoded != "" {
					// b64_json is raw Base64, while data_url is already a Data URL.
					if strings.HasPrefix(encoded, "data:") {
						reference.DataURL = encoded
					} else {
						...
						reference.DataURL = "data:" + mimeType + ";base64," + encoded
					}
				}
			}

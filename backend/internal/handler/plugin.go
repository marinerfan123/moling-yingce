...
func RegisterPluginRoutes(r *gin.RouterGroup, svc *service.Service) {
	...
	statusRoutes.GET("/status", func(c *gin.Context) {
		...
		statuses, err := svc.WorkflowPluginStatusesForActor(user)
		if err != nil {
			failService(c, err)
			return
		}
	...
	pluginRoutes.GET("/catalog", func(c *gin.Context) {
		...
		if scope == string(protocol.SurfaceUserCustomChannel) {
			if err := svc.RequireCustomChannelsForUser(user); err != nil {
				failService(c, err)
				return
			}
		}
...
func requireEnabledPlugin(c *gin.Context, svc *service.Service, pluginID string) (*model.User, bool) {
	user, err := currentUser(c, svc)
	if err == nil {
		err = svc.RequirePluginForActor(user, pluginID)
	}
	if err != nil {
		failService(c, err)
		return nil, false

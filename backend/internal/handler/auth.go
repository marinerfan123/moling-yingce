...
func RegisterAuthRoutes(r *gin.RouterGroup, svc *service.Service) {
	...
	r.POST("/auth/phone-code", func(c *gin.Context) {
		c.Request.Body = http.MaxBytesReader(c.Writer, c.Request.Body, 16<<10)
		var req struct {
		...
		policy, available := loadRuntimePolicy(c, svc)
		if !available || !enforceRateLimit(c, "phone-code:"+c.ClientIP(), policy.Request.EmailCodePerHour, time.Hour) {
			return
		}
		if !enforceRateLimit(c, "registration-phone-account:"+passwordResetRateLimitSubject(service.NormalizePhone(req.Phone)), 10, time.Hour) {
			return
		}
		if err := svc.SendRegistrationPhoneCodeContext(c.Request.Context(), req.Phone); err != nil {
			failService(c, err)
			return
		}
		ok(c, gin.H{"sent": true})
	...
	r.GET("/auth/wechat/start", socialLoginStartHandler(svc, "wechat"))
	r.GET("/auth/wechat/callback", socialLoginCallbackHandler(svc, "wechat"))
	r.GET("/auth/douyin/start", socialLoginStartHandler(svc, "douyin"))
	r.GET("/auth/douyin/callback", socialLoginCallbackHandler(svc, "douyin"))
...
func RegisterOAuthCallbackRoutes(r gin.IRoutes, svc *service.Service) {
	...
	r.GET("/oauth/wechat/callback", socialLoginCallbackHandler(svc, "wechat"))
	r.GET("/oauth/douyin/callback", socialLoginCallbackHandler(svc, "douyin"))
}

func socialLoginStartHandler(svc *service.Service, provider string) gin.HandlerFunc {
	return func(c *gin.Context) {
		if !enforceRateLimit(c, provider+"-start:"+c.ClientIP(), 20, 10*time.Minute) {
			return
		}
		target, err := svc.BeginSocialLogin(provider, c.Query("next"))
		if err != nil {
			failService(c, err)
			return
		}
...
func socialLoginCallbackHandler(svc *service.Service, provider string) gin.HandlerFunc {
	return func(c *gin.Context) {
		if !enforceRateLimit(c, provider+"-callback:"+c.ClientIP(), 30, 10*time.Minute) {
			return
		}
		result, err := svc.CompleteSocialLogin(provider, c.Query("state"), c.Query("code"))
		if err != nil {
...
func RegisterAdminRoutes(r *gin.RouterGroup, svc *service.Service) {
	...
	r.GET("/admin/channels/export", func(c *gin.Context) {
		user, err := currentUser(c, svc)
		if err != nil {
			failService(c, err)
			return
		}
		export, err := svc.ExportAdminSystemChannels(user)
		if err != nil {
			failService(c, err)
			return
		}
	...
	r.POST("/admin/channels/import", func(c *gin.Context) {
		user, err := currentUser(c, svc)
		if err != nil {
			failService(c, err)
			return
		...
		var req service.AdminChannelImportRequest
		if err := c.ShouldBindJSON(&req); err != nil {
			fail(c, http.StatusBadRequest, err)
			return
		}
		result, err := svc.ImportAdminSystemChannels(user, req)
		if err != nil {
			failService(c, err)
			return
		}
...
func proxySystemRequestPath(c *gin.Context, svc *service.Service, user *model.User, channel *model.ModelChannel, providerPath string) {
	...
	if channel.AllowLocalChannel && !svc.DesktopLocalChannelsEnabledForUser(user) {
		fail(c, http.StatusForbidden, errors.New("当前账号无权使用本机渠道"))
		return
	}
	path := providerPath
	if path == "" {

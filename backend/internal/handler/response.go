...
func failService(c *gin.Context, err error) {
	...
	var phoneCooldown *service.PhoneCodeCooldownError
	if errors.As(err, &phoneCooldown) {
		c.Header("Retry-After", strconv.Itoa(phoneCooldown.Seconds))
		writeFailure(c, http.StatusTooManyRequests, service.CodeRateLimited, service.ReasonRateLimited, phoneCooldown.Error())
		return
	}
